...
function payableFromTotals(t) {
  ...
  if (typeof t === 'object' && t.payable != null) {
    return Number(t.payable);
  }
  try {
    const o = typeof t === 'string' ? JSON.parse(t) : t;
    if (o && o.payable != null) {
      return Number(o.payable);
    }
  } catch {}
  return null;
}
async function markSaleFailedAndReleaseRewards(saleId) {
  const q = await pool.query(`UPDATE sales
       SET
       ...
       RETURNING id`, [saleId]);
  if (q.rowCount) {
    await releaseRewardsForSaleWithTransaction(saleId);
    return true;
  }
  return false;
}
router.post('/payments/create-order', async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO payments
         ...
         ON CONFLICT
           (razorpay_order_id)
         DO NOTHING`, [saleId, order.id, order.status || 'created', order.amount, order.currency, sale.customer_email || null, sale.customer_mobile || null, JSON.stringify(order.notes || {})]);
    return res.json({
...
router.post('/payments/verify', async (req, res) => {
  try {
    ...
    const p = await pool.query(`UPDATE payments
           SET
             razorpay_payment_id = $2,
             razorpay_signature = $3,
             status = $4
           WHERE razorpay_order_id = $1
           RETURNING sale_id`, [razorpay_order_id, razorpay_payment_id, razorpay_signature, status]);
    if (p.rowCount) {
      ...
      if (ok) {
        ...
        await markSaleFailedAndReleaseRewards(saleId);
      }
    }
    return res.json({
      ok,
...
router.post('/payments/mark-failed', async (req, res) => {
  try {
    ...
    const currentStatus = String(existing.rows[0].payment_status || '').toUpperCase();
    if (currentStatus === 'PAID') {
      return res.status(409).json({
        message: 'Paid sale cannot be marked failed'
      });
    }
    await markSaleFailedAndReleaseRewards(saleId);
    return res.json({
...
router.post('/razorpay/webhook', express.raw({
  ...
  try {
    ...
    if (eventId) {
      await pool.query(`INSERT INTO razorpay_webhook_events
           (
             event_id,
             event_type,
             payload
           ...
           ON CONFLICT
             (event_id)
           DO NOTHING`, [eventId, eventType, JSON.stringify(payload)]);
    }
    ...
    if (paymentEntity?.order_id) {
      ...
      await pool.query(`UPDATE payments
           ...
           WHERE razorpay_order_id = $1`, [paymentEntity.order_id, paymentEntity.id || null, status, paymentEntity.method || null, paymentEntity.email || null, paymentEntity.contact || null]);
      const saleQ = await pool.query(`SELECT sale_id
             FROM payments
             WHERE razorpay_order_id = $1
             LIMIT 1`, [paymentEntity.order_id]);
      if (saleQ.rowCount) {
        const saleId = saleQ.rows[0].sale_id;
        if (status === 'PAID') {
          await pool.query(`UPDATE sales
               SET
                 payment_status = 'PAID',
