...
const getOtpReference = value => crypto.createHash('sha256').update(String(value || '')).digest('hex');
...
async function createUser(req, res) {
  ...
  try {
    ...
    const insert = await client.query(`INSERT INTO ${USERS_TABLE}
         (
           name,
           email,
           mobile,
         ...
         RETURNING
           ...
           email,
           mobile,
           type`, [cleanName, cleanEmail, cleanMobile, hashedPassword, cleanType]);
    const user = insert.rows[0];
    if (String(user.type || 'B2C').toUpperCase() === 'B2C') {
      await creditSignupBonus(user.id, client);
    }
...
router.post('/change-password', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await pool.query(`UPDATE ${USERS_TABLE}
         SET
           password = $1,
           updated_at = NOW()
         WHERE id = $2`, [hashed, req.user.id]);
    return res.json({
...
router.post('/forgot/start', async (req, res) => {
  ...
  try {
    ...
    try {
      await client.query('BEGIN');
      await client.query(`UPDATE ${USERS_TABLE}
           SET
             otp = $1,
             otp_expiry = $2,
             otp_attempts = 0,
             ...
             updated_at = NOW()
           WHERE id = $3`, [otpHash, expiresAt, user.id]);
      await transporter.sendMail({
        from: process.env.FROM_EMAIL || process.env.SMTP_USER,
        to: cleanEmail,
        subject: 'V1 Garments password reset OTP',
        text: `Your V1 Garments password reset OTP is ${otp}. It is valid for 10 minutes.`,
        html: `<div style="font-family:Arial,sans-serif;color:#111;max-width:520px;margin:auto;padding:24px"><h2 style="margin:0 0 16px">Reset your V1 Garments password</h2><p>Use the OTP below to continue:</p><div style="font-size:32px;font-weight:700;letter-spacing:8px;margin:24px 0">${otp}</div><p>This OTP is valid for 10 minutes. Do not share it with anyone.</p></div>`
      });
      await client.query('COMMIT');
    } catch (err) {
      await client.query('ROLLBACK');
...
router.post('/forgot/verify', async (req, res) => {
  ...
  try {
    ...
    const isMatch = await bcrypt.compare(cleanOtp, String(user.otp || ''));
    if (!isMatch) {
      ...
      await client.query(`UPDATE ${USERS_TABLE}
           SET
             otp_attempts = $1,
             updated_at = NOW()
           WHERE id = $2`, [attempts, user.id]);
      await client.query('COMMIT');
      return res.status(400).json({
        message: attempts >= OTP_MAX_ATTEMPTS ? 'Too many incorrect attempts. Request a new OTP' : 'Invalid OTP',
...
router.post('/forgot/reset', async (req, res) => {
  ...
  const cleanResetToken = String(reset_token || resetToken || '').trim();
  const cleanNewPassword = String(newPassword || '').trim();
  if (!cleanResetToken || !cleanNewPassword) {
    return res.status(400).json({
  ...
  let decoded;
  try {
    decoded = jwt.verify(cleanResetToken, getPasswordResetSecret());
  } catch {
    return res.status(400).json({
      message: 'Reset session is invalid or expired'
    });
  }
  if (decoded?.purpose !== 'password-reset' || !decoded?.userId || !decoded?.email || !decoded?.otpRef) {
    return res.status(400).json({
      message: 'Reset session is invalid or expired'
    });
  }
  ...
  try {
    await client.query('BEGIN');
    const result = await client.query(`SELECT id, email, otp, otp_expiry
         FROM ${USERS_TABLE}
         WHERE id = $1
           AND lower(email) = $2
         LIMIT 1
         FOR UPDATE`, [decoded.userId, String(decoded.email).trim().toLowerCase()]);
    const user = result.rows[0];
    const validSession = user && user.otp && user.otp_expiry && new Date(user.otp_expiry).getTime() >= Date.now() && getOtpReference(user.otp) === decoded.otpRef;
    if (!validSession) {
      await client.query('ROLLBACK');
      return res.status(400).json({
        message: 'Reset session is invalid or expired'
  ...
  try {
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        ...
        const inserted = await client.query(`INSERT INTO ${USERS_TABLE}
               (
                 name,
                 email,
                 mobile,
               ...
               RETURNING
                 ...
                 email,
                 mobile,
                 type`, [displayName, cleanEmail, '', '', 'B2C']);
