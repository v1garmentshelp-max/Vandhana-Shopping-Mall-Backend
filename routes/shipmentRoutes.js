...
router.get('/shipments/by-sale/:id', async (req, res) => {
  ...
  try {
    const {
      rows
    } = await pool.query('SELECT * FROM shipments WHERE sale_id=$1 ORDER BY created_at ASC', [id]);
    const enriched = [];
    for (const sh of rows) {
      ...
      if (sh.shiprocket_order_id) {
        try {
          ...
          const updated = await syncShipmentRows(pool, [sh], {
            status: fallback
          }, fallback);
          current = updated[0] || sh;
        }
    ...
    const finalStatus = bestOrderStatus(enriched.flatMap(sh => [sh.status, sh.raw_status, sh.current_status, sh.shipment_status, sh.awb ? 'PACKED' : '', ...collectStatusValues(sh.last_tracking_payload)]), rows.length ? 'CONFIRMED' : 'PLACED');
