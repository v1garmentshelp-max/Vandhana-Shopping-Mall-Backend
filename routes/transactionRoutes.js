...
const mapTransactionRow = row => {
  ...
  const effectiveStatus = bestOrderStatus([row.status, row.effective_status, latestShipment?.status, latestShipment?.raw_status, latestShipment?.awb ? 'PACKED' : ''], row.status || 'PLACED');
  const codStatus = deriveCodStatus({
    ...row,
    effective_status: effectiveStatus
  }, latestShipment, latestCodRemittance);
  return {
    ...row,
...
router.get('/admin/:id', requireAuth, async (req, res) => {
  try {
    ...
    const q = await pool.query(`SELECT
       ...
       ${accessSql}
       LIMIT 1`, params);
    if (!q.rowCount) return res.status(404).json({
...
router.post('/cod-remittance/manual-settle', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!saleId) return res.status(400).json({
      message: 'sale_id required'
    });
    if (!requestedStatus) return res.status(400).json({
      message: 'remittance_status required'
    });
    ...
    if (paymentType !== 'COD') {
      ...
      return res.status(400).json({
        message: 'Manual COD remittance is allowed only for COD orders'
      });
    }
    const shipmentQ = await client.query(`SELECT *
       FROM shipments
    ...
    const existingQ = await client.query(`SELECT id
       FROM cod_remittances
       WHERE sale_id = $1::uuid
          OR ($2::text IS NOT NULL AND awb = $2)
       ORDER BY created_at DESC
       LIMIT 1`, [saleId, shipment?.awb || null]);
    let out = null;
    if (existingQ.rowCount) {
      const upd = await client.query(`UPDATE cod_remittances
         SET shipment_id = COALESCE($2::uuid, shipment_id),
             ...
             updated_at = now()
         WHERE id = $1
         RETURNING *`, [existingQ.rows[0].id, shipment?.id || null, shipment?.awb || '', shipment?.shiprocket_order_id || '', shipment?.shiprocket_shipment_id || '', amount, requestedStatus, utr || '', remittanceDate, scheduledFrom, scheduledTo, JSON.stringify({
        source: 'manual_admin',
      ...
      const ins = await client.query(`INSERT INTO cod_remittances
         ...
         RETURNING *`, [saleId, shipment?.id || null, shipment?.awb || null, shipment?.shiprocket_order_id || null, shipment?.shiprocket_shipment_id || null, amount, requestedStatus, utr, remittanceDate, scheduledFrom, scheduledTo, JSON.stringify({
        source: 'manual_admin',
        user_id: req.user?.id || null,
        body: req.body || {}
      })]);
