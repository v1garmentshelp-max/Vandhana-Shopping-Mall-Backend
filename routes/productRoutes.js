...
const normalizeBarcodeForWrite = v => String(v ?? '').trim().replace(/^"|"$/g, '').replace(/\s+/g, '').toUpperCase().replace(/[^A-Z0-9._-]/g, '');
...
const normalizeText = str => String(str || '').toLowerCase().replace(/₹/g, ' ').replace(/rs\.?/g, ' ').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim();
const STOPWORDS = new Set(['for', 'and', 'with', 'in', 'the', 'a', 'an', 'of', 'to', 'on', 'from', 'at', 'by', 'rs', 'rupees', 'below', 'under', 'upto', 'up', 'between', 'above', 'over', 'less', 'more', 'than', 'price']);
...
const storefrontCache = (req, res) => {
  const bypass = String(req.query.include_out_of_stock || req.query.includeOutOfStock || '').toLowerCase() === 'true' || String(req.query.include_grouped_values || req.query.includeGroupedValues || '').toLowerCase() === 'true' || String(req.query.no_cache || req.query.noCache || '').toLowerCase() === 'true';
  if (bypass) {
    noStore(res);
    return;
  }
...
const groupProductRows = (rows, {
  includeGroupedValues = false,
  groupBy = 'design'
} = {}) => {
  const groups = new Map();
  const normalizedGroupBy = ['color', 'colour'].includes(String(groupBy || '').trim().toLowerCase()) ? 'color' : 'design';
  ...
  for (const group of groups.values()) {
    group.variants.sort((a, b) => {
      const colorCompare = String(a.color || '').localeCompare(String(b.color || ''), undefined, {
        numeric: true
      });
    ...
    const sizes = uniqueValues(group.variants.map(v => v.size)).sort(compareSizes);
...
const resolveVariantForWrite = async ({
  client,
  id,
  variantIdFromBody,
  barcodeFromBody,
  ...
  if (Number.isFinite(bodyVariantId) && bodyVariantId > 0) {
    const byBodyVariant = await client.query(`SELECT v.id AS variant_id, v.product_id
       FROM product_variants v
       WHERE v.id = $1
       LIMIT 1`, [bodyVariantId]);
    if (byBodyVariant.rows.length) return byBodyVariant.rows[0];
  }
  if (barcode) {
    const byBarcode = await client.query(`SELECT v.id AS variant_id, v.product_id
       FROM barcodes b
  ...
  if (mode === 'variant') {
    const byVariant = await client.query(`SELECT v.id AS variant_id, v.product_id
       FROM product_variants v
       WHERE v.id = $1
       LIMIT 1`, [numericId]);
  ...
  if (mode === 'product' || mode === 'auto') {
    const byProduct = await client.query(`SELECT v.id AS variant_id, v.product_id
       FROM product_variants v
...
const getProductCategoryId = async (client, productId) => {
  const {
    rows
  } = await client.query(`SELECT category_id FROM products WHERE id = $1 LIMIT 1`, [productId]);
...
const validateCategoryForWrite = async (client, categoryId, gender, fallbackProductId) => {
  ...
  const {
    rows
  } = await client.query(`WITH RECURSIVE category_tree AS (
       SELECT
         c.id,
         c.parent_id,
         c.gender,
...
const saveProductImage = async (client, eanCode, imageUrl, imageType = 'front') => {
  ...
  try {
    await client.query(`INSERT INTO product_images (ean_code, image_type, image_url, public_id, uploaded_at)
       VALUES ($1, $2, $3, NULL, NOW())
       ON CONFLICT (ean_code, image_type)
       DO UPDATE SET image_url = EXCLUDED.image_url,
                     uploaded_at = NOW()`, [code, type, url]);
  } catch {}
};
const updateVariantRecord = async ({
  client,
  req,
  id,
  body,
  ...
  if (!nextName || !nextBrand || !nextSize || !nextColor) {
    return {
      status: 400,
      payload: {
        message: 'Product name, brand, color and size are required'
      }
    };
  }
  if (hasGroupedVariantValue(nextSize)) {
    return {
      status: 400,
      payload: {
        message: 'Size must be one value only. Do not send a grouped size summary.'
      }
    };
  }
  ...
  const barcodeRow = await client.query(`SELECT ean_code
     FROM barcodes
     WHERE variant_id = $1
     ORDER BY id ASC
     LIMIT 1`, [variantId]);
  const eanCode = normalizeBarcodeForWrite(body?.ean_code || body?.eanCode || body?.barcode || barcodeRow.rows[0]?.ean_code || '');
  const productMetadata = await client.query(`SELECT design_code, pattern_type, pattern_code
     FROM products
     WHERE id = $1
     LIMIT 1`, [productId]);
  ...
  if (requestedDesignCode && requestedDesignCode !== existingDesignCode) {
    return {
      status: 409,
      payload: {
        message: 'design_code cannot be changed through the product update endpoint'
      }
    };
  }
  ...
  const nextPatternType = hasPatternType ? normalizePatternType(body?.pattern_type ?? body?.patternType) || null : existingMetadata.pattern_type || null;
  await client.query(`UPDATE products
     SET name = $1,
         ...
         pattern_type = $5,
         updated_at = NOW()
     WHERE id = $6`, [nextName, nextBrand, gender, categoryId, nextPatternType, productId]);
  await client.query(`UPDATE product_variants
     SET colour = $1,
         ...
         is_active = TRUE,
         updated_at = NOW()
     WHERE id = $9`, [nextColor, nextSize, originalB2C, finalB2C, originalB2B, b2bDiscount, b2cDiscount, imageUrl, variantId]);
  if (eanCode) {
  ...
  const branchId = getBranchIdFromReq(req);
  if (branchId) {
    await client.query(`INSERT INTO branch_variant_stock (variant_id, branch_id, on_hand, reserved, is_active)
       VALUES ($1, $2, $3, 0, TRUE)
       ON CONFLICT (branch_id, variant_id)
       DO UPDATE SET on_hand = EXCLUDED.on_hand,
                     is_active = TRUE,
                     updated_at = NOW()`, [variantId, branchId, stockCount]);
  }
...
const deleteVariantById = async ({
  ...
  const existingVariant = await client.query(`SELECT v.id, v.product_id, v.size, v.colour
     FROM product_variants v
     JOIN products p ON p.id = v.product_id
     WHERE v.id = $1
     FOR UPDATE OF p, v`, [variantId]);
  ...
  const barcodeRows = await client.query(`SELECT TRIM(ean_code) AS ean_code
     FROM barcodes
     WHERE variant_id = $1`, [variantId]);
  const eanCodes = barcodeRows.rows.map(row => cleanValue(row.ean_code)).filter(Boolean);
  if (eanCodes.length) {
    await client.query(`DELETE FROM product_images
       WHERE TRIM(ean_code) = ANY($1::text[])`, [eanCodes]);
  }
  await client.query(`DELETE FROM product_design_mapping_review
     WHERE variant_id = $1`, [variantId]);
  await client.query(`DELETE FROM product_variants
     WHERE id = $1`, [variantId]);
  const remainingVariants = await client.query(`SELECT 1
     FROM product_variants
     WHERE product_id = $1
     LIMIT 1`, [variant.product_id]);
  let productDeleted = false;
  if (!remainingVariants.rows.length) {
    await client.query(`DELETE FROM product_design_mapping_review
       WHERE product_id = $1`, [variant.product_id]);
    await client.query(`DELETE FROM products
       WHERE id = $1`, [variant.product_id]);
    productDeleted = true;
  }
  return {
    status: 200,
    payload: {
      message: productDeleted ? 'Variant and product permanently deleted successfully' : 'Variant permanently deleted successfully',
      variant_id: variantId,
...
router.get('/category-previews', async (req, res) => {
  const branchId = Number(req.query.branch_id || WEB_BRANCH_ID || 3);
...
`, [branchId]);
    ...
    console.error('Category previews failed:', error.message);
    return res.status(500).json({
      message: 'Unable to load category previews'
    });
  }
});
...
router.get('/suggest', async (req, res) => {
  try {
    ...
    for (const row of rows) {
      const values = [row.product_name, row.brand, row.color, row.category_name, row.parent_category_name, row.category_path, row.gender ? `${row.gender} ${row.product_name}` : ''];
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (!queryRaw || !String(queryRaw).trim()) return res.status(400).json({
...
router.get('/section-images', async (req, res) => {
  try {
    ...
    const hero = await fetchProducts({
      req,
      limit: String(limitHero),
      offset: '0',
      random: true,
      hasImage: true
    });
...
router.delete('/variant/:variantId(\\d+)', async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(variantId) || variantId <= 0) return res.status(400).json({
      message: 'Invalid variant id'
    });
    await client.query('BEGIN');
    const result = await deleteVariantById({
      client,
      req,
      variantId
    });
...
const updateBarcodeHandler = async (req, res) => {
  ...
  try {
    ...
    if (!barcode) return res.status(400).json({
...
router.delete('/:id(\\d+)', async (req, res) => {
  ...
  try {
    ...
    const product = await client.query(`SELECT id
       FROM products
       WHERE id = $1
       FOR UPDATE`, [id]);
    if (product.rows.length) {
      const variants = await client.query(`SELECT id
         FROM product_variants
         WHERE product_id = $1
         ORDER BY id`, [id]);
      const variantIds = variants.rows.map(r => r.id);
      const barcodeRows = variantIds.length ? await client.query(`SELECT TRIM(ean_code) AS ean_code
             FROM barcodes
             WHERE variant_id = ANY($1::bigint[])`, [variantIds]) : {
        rows: []
      };
      const eanCodes = barcodeRows.rows.map(row => cleanValue(row.ean_code)).filter(Boolean);
      if (eanCodes.length) {
      ...
      await client.query(`DELETE FROM product_design_mapping_review
         WHERE product_id = $1
            OR variant_id = ANY($2::bigint[])`, [id, variantIds]);
      await client.query(`DELETE FROM products
         WHERE id = $1`, [id]);
      await client.query('COMMIT');
      return res.json({
