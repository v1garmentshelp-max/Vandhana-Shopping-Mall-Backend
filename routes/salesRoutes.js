...
const resolvePosBranchId = req => {
  ...
  if (role === 'SUPER_ADMIN') return requestedBranchId || authBranchId
  return authBranchId
}
...
const resolvePosItems = async (client, rawItems) => {
  ...
  for (const rawItem of rawItems) {
    ...
    if (!qty) throw Object.assign(new Error('qty must be a positive integer'), { status: 400 })
    ...
    if (!eanCode && !requestedVariantId) {
      throw Object.assign(new Error('ean_code or variant_id required for every POS item'), { status: 400 })
    }

    const snapshot = eanCode
      ? await resolveActiveVariantByBarcode(client, eanCode)
      : await resolveActiveVariantById(client, requestedVariantId)
...
const lockAndDeductPosStock = async (client, branchId, items) => {
  ...
  for (const item of items) {
    const stockQ = await client.query(
      `SELECT on_hand, reserved, is_active
       FROM branch_variant_stock
       WHERE branch_id = $1
         AND variant_id = $2
       FOR UPDATE`,
      [branchId, item.variant_id]
    )
    ...
    const updateQ = await client.query(
      `UPDATE branch_variant_stock
       ...
       WHERE branch_id = $1
         AND variant_id = $2
       RETURNING on_hand, reserved, GREATEST(on_hand - reserved, 0)::int AS available_qty`,
      [branchId, item.variant_id, item.qty]
    )
...
router.post('/pos/place', requireAuth, async (req, res) => {
  ...
  if (!isPosPaymentMethod(paymentMethod)) {
    return res.status(400).json({ message: 'payment_method must be POS_CASH, POS_CARD, POS_UPI or POS_OTHER' })
  }

  if (!rawItems.length) return res.status(400).json({ message: 'items required' })
  ...
  try {
    ...
    const branchQ = await client.query('SELECT id FROM branches WHERE id=$1 LIMIT 1', [branchId])
    if (!branchQ.rowCount) {
    ...
    const storedTotals = { ...suppliedTotals, ...calculatedTotals, pos: true }

    const saleQ = await client.query(
      `INSERT INTO sales
       (source, status, payment_status, payment_method, total, totals, branch_id, customer_name, customer_email, customer_mobile, shipping_address, login_email, is_b2b, created_at, updated_at)
       VALUES
       ('POS', 'COMPLETED', 'PAID', $1, $2, $3::jsonb, $4, $5, $6, $7, $8::jsonb, $9, false, NOW(), NOW())
       RETURNING id, source, status, payment_status, payment_method, total, branch_id, created_at`,
      [
        paymentMethod,
        calculatedTotals.payable,
        JSON.stringify(storedTotals),
        branchId,
        cleanText(body.customer_name || body.customerName) || null,
        cleanText(body.customer_email || body.customerEmail) || null,
        cleanText(body.customer_mobile || body.customerMobile) || null,
        JSON.stringify(body.shipping_address && typeof body.shipping_address === 'object' ? body.shipping_address : {}),
        cleanText(body.login_email || body.loginEmail) || null
      ]
    )
    ...
    for (const item of resolvedItems) {
      await client.query(
        `INSERT INTO sale_items
         (id, sale_id, product_id, variant_id, qty, price, mrp, size, colour, image_url, ean_code, created_at)
         VALUES
    ...
    if (error?.available_qty != null) payload.available_qty = Number(error.available_qty)
    if (error?.item) {
      payload.item = {
...
router.post('/web/place', async (req, res) => {
  ...
  try {
    ...
    if (trustedVariants.size !== variantIds.length) {
      ...
      await client.query('ROLLBACK')
      return res.status(409).json({ message: `Inactive or missing variant: ${missing.join(', ')}` })
    }
      ...
      const branchQ = await client.query(
        `WITH cart AS (
         ...
         WHERE bvs.is_active = TRUE
           AND v.is_active = TRUE
           AND p.is_active = TRUE
           AND GREATEST(COALESCE(bvs.on_hand, 0) - COALESCE(bvs.reserved, 0), 0) >= c.qty
    ...
    if (!resolvedBranchId) {
      await client.query('ROLLBACK')
      return res.status(409).json({ message: 'Stock not available in a single branch for all items' })
    }
    ...
    for (const vId of variantIds) {
      ...
      if (!stockQ.rowCount) {
        await client.query('ROLLBACK')
        return res.status(409).json({ message: `Stock not found for variant ${vId} in branch ${resolvedBranchId}` })
      }
    ...
    for (const it of items) {
      ...
      await client.query(
        `INSERT INTO sale_items
         (id, sale_id, product_id, variant_id, qty, price, mrp, size, colour, image_url, ean_code)
         VALUES
         ($1::uuid,$2::uuid,$3,$4,$5,$6,$7,$8,$9,$10,$11)`,
        [
          uuid(),
          saleId,
          Number(trusted.product_id),
          vId,
          qty,
          Number(it?.price ?? 0) || 0,
          it?.mrp != null ? Number(it.mrp) : trusted.mrp != null ? Number(trusted.mrp) : null,
          trusted.size ?? it?.size ?? it?.selected_size ?? null,
          trusted.colour ?? it?.colour ?? it?.color ?? it?.selected_color ?? null,
          it?.image_url ?? null,
          trusted.ean_code || it?.ean_code || it?.barcode_value || null
        ]
      )
    }
