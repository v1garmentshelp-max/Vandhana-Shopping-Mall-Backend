...
router.use('/:id', async (req, res, next) => {
  try {
    ...
    if (role !== 'SUPER_ADMIN' && String(req.user.branch_id) !== String(sale.branch_id)) return res.status(403).json({ message: 'This order belongs to another branch' })
...
const run = fn => async (req, res) => {
  try { res.json(await fn(req)) }
    ...
    res.status(e.status >= 400 && e.status <= 599 ? e.status : 502).json({ message })
  }
}
...
router.get('/:id/pickups', run(async () => {
  const { data } = await new Shiprocket({ pool }).api('get', '/settings/company/pickup')
  return { pickups: data?.data?.shipping_address || [] }
}))
router.post('/:id/warehouse', run(async req => {
  if ((await workflow.state(req.params.id)).shipment) throw Object.assign(new Error('A shipment already exists. Change its pickup location in Shiprocket, then reconcile it.'), { status: 409 })
  const { data } = await new Shiprocket({ pool }).api('get', '/settings/company/pickup')
  const pickup = data?.data?.shipping_address?.find(p => String(p.id || p.pickup_id) === String(req.body.pickup_id))
  if (!pickup?.pickup_location || !/^\d{6}$/.test(String(pickup.pin_code || ''))) throw Object.assign(new Error('Select a valid existing Shiprocket pickup location.'), { status: 422 })
  await pool.query(`INSERT INTO shiprocket_warehouses(branch_id,warehouse_id,name,pincode,city,state,address,phone)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(branch_id) DO UPDATE SET warehouse_id=EXCLUDED.warehouse_id,name=EXCLUDED.name,pincode=EXCLUDED.pincode,city=EXCLUDED.city,state=EXCLUDED.state,address=EXCLUDED.address,phone=EXCLUDED.phone`,
  [req.shippingSale.branch_id, pickup.id || pickup.pickup_id, pickup.pickup_location, pickup.pin_code, pickup.city, pickup.state, pickup.address, pickup.phone || ''])
