...
router.get('/admin/summary', requireAuth, async (req, res) => {
  try {
    ...
    const activeQ = await pool.query(`SELECT
         ...
         WHERE status = 'ACTIVE'
           ...
           AND expires_at > NOW()`, [settings.warning_days]);
    const customerQ = await pool.query(`SELECT
           COUNT(*)::int AS total_b2c_customers
         FROM vandana_users
         WHERE UPPER(COALESCE(type, 'B2C')) = 'B2C'`);
    const transactionQ = await pool.query(`SELECT
           COALESCE(
             SUM(-points)
             FILTER (
    ...
    const lotsQ = await pool.query(`SELECT
    ...
    return res.json({
      settings,
      metrics: {
        total_b2c_customers: Number(customerQ.rows[0]?.total_b2c_customers || 0),
        users_with_active_points: Number(active.users_with_active_points || 0),
        active_points: Number(active.active_points || 0),
        expiring_soon_points: Number(active.expiring_soon_points || 0),
        redeemed_points: Number(transaction.redeemed_points || 0),
        expired_points: Number(transaction.expired_points || 0),
        refunded_points: Number(transaction.refunded_points || 0),
        total_transactions: Number(transaction.total_transactions || 0),
        total_lots: Number(lots.total_lots || 0),
        active_lots: Number(lots.active_lots || 0),
        used_lots: Number(lots.used_lots || 0),
        expired_lots: Number(lots.expired_lots || 0)
      }
    });
  } catch (error) {
...
router.get('/admin/users', requireAuth, async (req, res) => {
  try {
    ...
    const q = await pool.query(`WITH active_lots AS (
           SELECT
             user_id,
         ...
         ORDER BY
           ...
           u.id DESC
         LIMIT $2
         OFFSET $3`, [search, limit, offset, settings.warning_days]);
    ...
    return res.json({
      ...
      users: q.rows.map(row => ({
        ...
        expiring_soon_points: Number(row.expiring_soon_points || 0),
        nearest_expiry: row.nearest_expiry,
        days_remaining: row.days_remaining == null ? null : Number(row.days_remaining),
        hurry_up: Boolean(row.hurry_up),
        transaction_count: Number(row.transaction_count || 0),
        redeemed_points: Number(row.redeemed_points || 0)
      }))
    });
  } catch (error) {
    return res.status(500).json({
...
router.get('/admin/users/:userId', requireAuth, async (req, res) => {
  try {
    ...
    const userQ = await pool.query(`SELECT
           id,
           name,
           email,
           mobile,
    ...
    const wallet = await getWalletSummary(userId, pool);
    const history = await getHistory(userId, 100, pool);
    return res.json({
      user: userQ.rows[0],
      wallet,
      history
...
router.get('/wallet', requireCustomerAuth, async (req, res) => {
  try {
    const wallet = await getWalletSummary(req.customer.id);
...
router.get('/history', requireCustomerAuth, async (req, res) => {
  try {
    const history = await getHistory(req.customer.id, req.query.limit);
    return res.json({
      history
    });
  } catch (error) {
...
router.post('/preview', requireCustomerAuth, async (req, res) => {
  try {
    const result = await previewRedemption({
      userId: req.customer.id,
      requestedPoints: req.body?.reward_points ?? req.body?.points ?? 0,
      orderSubtotal: req.body?.order_subtotal ?? req.body?.subtotal ?? req.body?.payable ?? 0
    });
    ...
    if (error?.available_points != null) {
      payload.available_points = Number(error.available_points);
    }
    if (error?.max_redeemable != null) {
      payload.max_redeemable = Number(error.max_redeemable);
    }
