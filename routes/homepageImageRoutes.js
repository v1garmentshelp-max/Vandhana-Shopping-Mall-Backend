...
function uploadSingleImage(req, res, next) {
  upload.single('image')(req, res, error => {
    ...
    if (error.code === 'LIMIT_FILE_SIZE') return res.status(413).json({
      message: 'Image must be smaller than 3.5 MB'
    });
...
router.get('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(`SELECT id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,created_at,updated_at
       FROM homepage_images
       ${where}
       ORDER BY COALESCE(page,''),COALESCE(section,''),slot_order,id`, values);
    return res.json(result.rows.map(shapeImage));
  } catch (error) {
    console.error('homepage images load failed', error);
    return res.status(500).json({
      message: 'Failed to load homepage images'
    });
  }
});
router.get('/settings', async (req, res) => {
  ...
  try {
    const result = await pool.query(`SELECT page,section,is_enabled,updated_by,created_at,updated_at
       FROM homepage_section_settings
       ${where}
       ORDER BY page,section`, values);
    return res.json(result.rows.map(shapeSectionSetting));
  } catch (error) {
    console.error('homepage section settings load failed', error);
    return res.status(500).json({
      message: 'Failed to load homepage section settings'
    });
  }
});
router.patch('/settings/:page/:section', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO homepage_section_settings(page,section,is_enabled,updated_by,updated_at)
       VALUES($1,$2,$3,$4,NOW())
       ON CONFLICT(page,section) DO UPDATE SET
       is_enabled=EXCLUDED.is_enabled,
       updated_by=EXCLUDED.updated_by,
       updated_at=NOW()
       RETURNING page,section,is_enabled,updated_by,created_at,updated_at`, [page, section, enabled, req.user.id]);
    return res.json(shapeSectionSetting(result.rows[0]));
  } catch (error) {
    console.error('homepage section setting update failed', error);
    return res.status(500).json({
      message: 'Failed to update homepage section setting'
    });
  }
});
router.get('/:id/history', requireAuth, async (req, res) => {
  ...
  if (!id) return res.status(400).json({
    message: 'Missing image slot id'
  });
  try {
    const result = await pool.query(`SELECT h.id,h.homepage_image_id,h.image_url,h.alt_text,h.link,h.extra_json,h.changed_by,h.change_type,h.created_at,u.username AS changed_by_username,u.name AS changed_by_name
...
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO homepage_images(id,page,section,slot_order,default_image_url,alt_text,link,extra_json,updated_by,updated_at)
       ...
       updated_by=EXCLUDED.updated_by,
       updated_at=NOW()
       RETURNING id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,created_at,updated_at`, [id, page, section, slotOrder, defaultImageUrl, altText, link, jsonValue(extra), req.user.id]);
    return res.json(shapeImage(result.rows[0]));
  } catch (error) {
    console.error('homepage image metadata update failed', error);
    return res.status(500).json({
      message: 'Failed to update homepage image metadata'
    });
  }
});
router.post('/:id/replace', requireAuth, uploadSingleImage, async (req, res) => {
  ...
  if (!CLOUD_NAME || !UPLOAD_PRESET) return res.status(500).json({
  ...
  try {
    ...
    const currentResult = await client.query(`SELECT id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,created_at,updated_at
       FROM homepage_images
       WHERE id=$1
       FOR UPDATE`, [id]);
    let current = currentResult.rows[0] || null;
    if (!current) {
      const insertedResult = await client.query(`INSERT INTO homepage_images(id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,updated_at)
    ...
    if (previousImageUrl && previousImageUrl !== uploaded.imageUrl) {
      await client.query(`INSERT INTO homepage_image_history(homepage_image_id,image_url,alt_text,link,extra_json,changed_by,change_type)
         VALUES($1,$2,$3,$4,$5::jsonb,$6,'REPLACE')`, [id, previousImageUrl, current.alt_text || altText, current.link || link, jsonValue(current.extra_json || extra), req.user.id]);
    }
    const updatedResult = await client.query(`UPDATE homepage_images SET
       ...
       updated_at=NOW()
       WHERE id=$1
       RETURNING id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,created_at,updated_at`, [id, page, section, slotOrder, uploaded.imageUrl, defaultImageUrl, altText, link, jsonValue(uploadExtra), req.user.id]);
    ...
    return res.status(500).json({
      message: 'Failed to replace homepage image'
    });
  } finally {
    client.release();
  }
...
router.post('/:id/history/:historyId/restore', requireAuth, async (req, res) => {
  ...
  if (!id || !Number.isInteger(historyId) || historyId <= 0) return res.status(400).json({
    message: 'Invalid restore request'
  });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const currentResult = await client.query(`SELECT id,image_url,default_image_url,alt_text,link,extra_json
       FROM homepage_images
       WHERE id=$1
       FOR UPDATE`, [id]);
    if (!currentResult.rows.length) {
      ...
      return res.status(404).json({
        message: 'Homepage image slot not found'
      });
    }
    const historyResult = await client.query(`SELECT id,image_url,alt_text,link,extra_json
       FROM homepage_image_history
       WHERE id=$1 AND homepage_image_id=$2`, [historyId, id]);
    if (!historyResult.rows.length) {
      await client.query('ROLLBACK');
      return res.status(404).json({
        message: 'Previous image version not found'
      });
    }
    ...
    if (current.image_url && current.image_url !== historical.image_url) {
      await client.query(`INSERT INTO homepage_image_history(homepage_image_id,image_url,alt_text,link,extra_json,changed_by,change_type)
         VALUES($1,$2,$3,$4,$5::jsonb,$6,'RESTORE_BACKUP')`, [id, current.image_url, current.alt_text, current.link, jsonValue(current.extra_json), req.user.id]);
    }
    const updatedResult = await client.query(`UPDATE homepage_images SET
       image_url=$2,
       alt_text=$3,
       ...
       updated_at=NOW()
       WHERE id=$1
       RETURNING id,page,section,slot_order,image_url,default_image_url,alt_text,link,extra_json,updated_by,created_at,updated_at`, [id, historical.image_url, historical.alt_text, historical.link, jsonValue(historical.extra_json), req.user.id]);
    await client.query('COMMIT');
    return res.json(shapeImage(updatedResult.rows[0]));
  } catch (error) {
    await client.query('ROLLBACK');
    ...
    return res.status(500).json({
      message: 'Failed to restore homepage image'
    });
  } finally {
    client.release();
  }
