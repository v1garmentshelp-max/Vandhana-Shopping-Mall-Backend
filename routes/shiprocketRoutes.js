  ...
  if (typeof input === 'object') {
    for (const [key, value] of Object.entries(input)) {
      ...
      if (k.includes('status') || k.includes('activity') || k.includes('remark') || k.includes('description') || k.includes('event') || k.includes('scan')) {
        if (typeof value === 'string' || typeof value === 'number') out.push(String(value));else collectStatusValues(value, depth + 1, out);
...
const extractTrackingData = tracking => {
  ...
  if (Array.isArray(data) && data.length) {
    ...
    if (first && typeof first === 'object') {
      ...
      if (key && first[key]?.tracking_data) data = first[key].tracking_data;else data = first;
    }
  }
  if (data?.tracking_data) data = data.tracking_data;else if (data?.data?.tracking_data) data = data.data.tracking_data;else if (data?.data?.data?.tracking_data) data = data.data.data.tracking_data;else if (data?.data) data = data.data;
...
const extractShipmentInfo = (raw, fallbackStatus = '') => {
  ...
  const rawStatus = findFirstByKeys(data, ['current_status', 'shipment_status', 'status', 'track_status', 'activity']) || statuses[0] || fallbackStatus || '';
  ...
  const remittanceStatus = findFirstByKeys(data, ['remittance_status', 'cod_remittance_status', 'settlement_status', 'payment_remittance_status']) || null;
  const remittanceUtr = findFirstByKeys(data, ['remittance_utr', 'utr', 'utr_number', 'transaction_reference']) || null;
  const remittanceDate = findFirstByKeys(data, ['remittance_date', 'settlement_date', 'paid_date', 'payment_date']) || null;
  const scheduledFrom = findFirstByKeys(data, ['remittance_scheduled_from', 'scheduled_from']) || null;
  const scheduledTo = findFirstByKeys(data, ['remittance_scheduled_to', 'scheduled_to']) || null;
  const codAmount = findFirstByKeys(data, ['cod_amount', 'collectable_amount', 'collected_amount', 'remittance_amount', 'amount']) || null;
  return {
    raw: data,
    statuses,
...
const syncSaleStatus = async (saleId, candidateStatus) => {
  ...
  if (shouldUpdateStatus(currentStatus, finalStatus)) {
    const upd = await pool.query('UPDATE sales SET status=$2, updated_at=now() WHERE id=$1::uuid RETURNING status', [saleId, finalStatus]);
...
const syncCodRemittance = async (saleId, shipment, info, raw) => {
  ...
  const saleQ = await pool.query('SELECT id, payment_status, payment_method, total, totals FROM sales WHERE id=$1::uuid LIMIT 1', [saleId]);
  ...
  const existing = await pool.query(`SELECT id, remittance_status
  ...
  if (existing.rowCount) {
    const q = await pool.query(`UPDATE cod_remittances
       SET shipment_id = COALESCE($2::uuid, shipment_id),
           ...
           updated_at = now()
       WHERE id = $1
       RETURNING *`, [existing.rows[0].id, shipment?.id || null, awb || '', shiprocketOrderId || '', shiprocketShipmentId || '', amount, remittanceStatus, info.remittance_utr || '', remittanceDate, scheduledFrom, scheduledTo, JSON.stringify(raw || info.raw || {})]);
    return q.rows[0] || null;
  }
  const q = await pool.query(`INSERT INTO cod_remittances
     ...
     RETURNING *`, [saleId, shipment?.id || null, awb, shiprocketOrderId, shiprocketShipmentId, amount, remittanceStatus, info.remittance_utr || null, remittanceDate, scheduledFrom, scheduledTo, JSON.stringify(raw || info.raw || {})]);
  return q.rows[0] || null;
};
const syncShipmentRows = async (rows, raw, fallbackStatus = '') => {
  ...
  for (const row of rows || []) {
    const finalStatus = bestOrderStatus([row.status, info.status, info.raw_status, info.awb ? 'PACKED' : '', ...info.statuses], row.status || fallbackStatus || 'CONFIRMED');
    ...
    const q = await pool.query(`UPDATE shipments
       SET status = COALESCE($2, status),
           ...
           updated_at = now()
       WHERE id = $1
       RETURNING *`, [row.id, nextStatus || null, info.raw_status || '', info.awb || '', info.tracking_url || '', info.label_url || '', info.current_location || '', JSON.stringify(info.raw || raw || {}), deliveredAt]);
...
const syncShipmentRowsForSale = async (saleId, candidateStatus, raw = null) => {
  ...
  const updated = await syncShipmentRows(rows, raw || {
    status
  }, status);
  await syncSaleStatus(saleId, status);
  return updated;
...
const insertWebhookEvent = async ({
  ...
  try {
    await pool.query(`INSERT INTO shiprocket_webhook_events
       ...
       VALUES ($1::uuid,$2::uuid,$3,$4,$5,$6,$7,$8::jsonb,$9,$10)`, [saleId || info.sale_id || null, shipmentId || null, info.shiprocket_order_id || null, info.shiprocket_shipment_id || null, info.awb || null, info.raw_status || null, info.status || null, JSON.stringify(payload || {}), !!processed, errorMessage || null]);
  } catch {}
};
...
router.get('/shiprocket/warehouses', async (req, res) => {
  try {
    const {
      rows
    } = await pool.query('SELECT id, branch_id, warehouse_id, name, pincode, city, state, address, phone, created_at, updated_at FROM shiprocket_warehouses ORDER BY id ASC');
    res.json(rows);
  } catch {
    res.status(500).json({
      ok: false,
...
router.post('/shiprocket/warehouses/import', async (req, res) => {
  try {
    ...
    const {
      data
    } = await sr.api('get', '/settings/company/pickup');
    const pickups = Array.isArray(data?.data?.shipping_address) ? data.data.shipping_address : [];
    const {
      rows: branches
    } = await pool.query('SELECT id, name, address, city, state, pincode, phone FROM branches WHERE is_active = true');
    ...
    for (const b of branches) {
      ...
      if (!best) {
        results.push({
          branch_id: b.id,
          error: 'No matching pickup found in Shiprocket'
        });
        continue;
      }
      ...
      await pool.query(`INSERT INTO shiprocket_warehouses (branch_id, warehouse_id, name, pincode, city, state, address, phone)
         ...
         SET warehouse_id=EXCLUDED.warehouse_id,
             ...
             phone=EXCLUDED.phone`, [b.id, pickupId, pickupName, String(best.pin_code || b.pincode || ''), best.city || b.city || '', best.state || b.state || '', best.address || b.address || '', b.phone || '']);
      results.push({
        branch_id: b.id,
        mapped_to: pickupName,
        pickup_id: pickupId
      });
    }
    res.json({
...
router.post('/shiprocket/warehouses/sync', async (req, res) => {
  try {
    ...
    const {
      rows: branches
    } = await pool.query('SELECT id, name, address, city, state, pincode, phone, email FROM branches WHERE is_active = true');
    const results = [];
    for (const b of branches) {
      try {
        ...
        await pool.query(`INSERT INTO shiprocket_warehouses (branch_id, warehouse_id, name, pincode, city, state, address, phone)
           ...
           SET warehouse_id=EXCLUDED.warehouse_id, name=EXCLUDED.name, pincode=EXCLUDED.pincode, 
               city=EXCLUDED.city, state=EXCLUDED.state, address=EXCLUDED.address, phone=EXCLUDED.phone`, [b.id, data?.pickup_id || 0, pickupName, b.pincode, b.city, b.state, b.address, b.phone]);
        results.push({
          branch_id: b.id,
          pickup: pickupName
        });
...
router.post('/shiprocket/fulfill/:id', (req, res) => {
  res.status(409).json({
    ok: false,
    message: 'Use Connect order in the updated admin. Legacy fulfillment is disabled to prevent duplicate stock deductions.'
  });
});
...
async function computeServiceabilityForSaleId(saleId) {
  ...
  if (!pickupPin) {
    const {
      rows
    } = await pool.query('SELECT pincode FROM branches WHERE is_active = true AND pincode IS NOT NULL LIMIT 1');
  ...
  if (!pickupPin || pickupPin.length !== 6) {
    return {
      error: {
        ...
        body: {
          ok: false,
          message: 'No pickup pincode configured'
        }
      }
    };
  }
...
async function getShipmentsForSale(saleId) {
  const {
    rows
  } = await pool.query('SELECT * FROM shipments WHERE sale_id=$1 ORDER BY created_at DESC', [saleId]);
  if (!rows.length) return {
    error: {
      code: 404,
      body: {
        ok: false,
        message: 'No shipments found for this sale'
      }
    }
  };
  ...
  if (!shipmentIds.length) return {
    error: {
      ...
      body: {
        ok: false,
        message: 'No Shiprocket shipment ids found'
      }
    }
  };
  return {
...
async function getLatestShiprocketOrderIdForSale(saleId) {
  const {
    rows
  } = await pool.query('SELECT shiprocket_order_id FROM shipments WHERE sale_id=$1 AND shiprocket_order_id IS NOT NULL ORDER BY created_at DESC LIMIT 1', [saleId]);
  ...
  if (!orderId) return {
    error: {
      ...
      body: {
        ok: false,
        message: 'Shiprocket order id not found for this sale'
      }
    }
  };
  return {
...
router.get('/shiprocket/pincode', async (req, res) => {
  try {
    ...
    if (!deliveryPin || deliveryPin.length !== 6) return res.status(400).json({
      ok: false,
    ...
    if (!pickupPin) return res.status(500).json({
      ok: false,
      message: 'No pickup pincode configured'
    });
    const sr = new Shiprocket({
...
router.post('/shiprocket/assign-courier/by-sale/:saleId', (req, res) => res.status(409).json({
  message: 'Use the updated order shipping panel to generate AWB safely.'
}));
router.post('/shiprocket/assign-courier', (req, res) => res.status(409).json({
  message: 'Use the updated order shipping panel to generate AWB safely.'
}));
router.post('/shiprocket/assign-awb/by-sale/:saleId', (req, res) => res.status(409).json({
  message: 'Use the updated order shipping panel to generate AWB safely.'
}));
router.post('/shiprocket/assign-awb', (req, res) => res.status(409).json({
  message: 'Use the updated order shipping panel to generate AWB safely.'
}));
router.get('/shiprocket/tracking/by-sale/:saleId', async (req, res) => {
  try {
    ...
    const {
      data
    } = await sr.api('get', `/courier/track?order_id=${encodeURIComponent(String(orderId))}`);
...
router.get('/shiprocket/label/:saleId', async (req, res) => {
  try {
    ...
    const shipmentIds = rows.map(r => r.shiprocket_shipment_id).filter(v => v != null);
    if (!shipmentIds.length) return res.status(404).json({
      ok: false,
      message: 'No Shiprocket shipment ids found'
    });
    const sr = new Shiprocket({
    ...
    await sr.init();
    const result = await sr.assignAWBAndLabel({
      shipment_id: shipmentIds
    });
    const labelUrl = result?.label?.label_url || result?.label_url || null;
    if (!labelUrl) return res.status(500).json({
      ok: false,
...
router.get('/shiprocket/invoice/:saleId', async (req, res) => {
  try {
    ...
    const {
      rows
    } = await pool.query('SELECT * FROM shipments WHERE sale_id=$1 ORDER BY created_at ASC', [saleId]);
    if (!rows.length) return res.status(404).json({
      ok: false,
      message: 'No shipments found for this sale'
    });
    const orderIds = Array.from(new Set(rows.map(r => r.shiprocket_order_id).filter(v => v != null)));
    if (!orderIds.length) return res.status(404).json({
      ok: false,
      message: 'No Shiprocket order ids found'
    });
    const sr = new Shiprocket({
      pool
    });
    ...
    const {
      data
    } = await sr.api('post', '/orders/print/invoice', {
      ids: orderIds
    });
    const invoiceUrl = data?.invoice_url || data?.data?.invoice_url || null;
    if (!invoiceUrl) return res.status(500).json({
      ok: false,
...
router.get('/shiprocket/manifest/:saleId', async (req, res) => {
  try {
    ...
    await sr.init();
    const data = await sr.generateManifest({
      shipment_ids: shipmentIds
    });
    const manifestUrl = data?.manifest_url || data?.data?.manifest_url || null;
    if (!manifestUrl) return res.status(500).json({
      ok: false,
