...
function normalizeDesignCode(value) {
  return cleanText(value).toUpperCase().replace(/\s+/g, '-').replace(/[^A-Z0-9._-]/g, '');
}
...
function normalizeBarcode(value) {
  return String(value ?? '').trim().replace(/^"|"$/g, '').replace(/\s+/g, '').toUpperCase().replace(/[^A-Z0-9._-]/g, '');
}
function normalizeImageType(value) {
  const normalized = String(value || '').toLowerCase().trim().replace(/[^a-z0-9_-]/g, '');
...
function isDefaultText(value) {
  ...
  const blockedValues = new Set(['brand', 'product', 'new in', 'inclusive of all taxes', '₹0.00', '0', '0.00', '₹0', '₹0.0']);
...
async function findProductByDesignCode(client, designCode, lock = false) {
  ...
  const result = await client.query(`SELECT id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id
     FROM products
     WHERE UPPER(TRIM(COALESCE(design_code, ''))) = $1
     ORDER BY id ASC
     LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [normalized]);
  return result.rows[0] || null;
}
async function findProductsByLegacyIdentity(client, prepared, categoryId, gender, lock = false) {
  const result = await client.query(`SELECT id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id
     ...
     ORDER BY id ASC
     LIMIT 3${lock ? ' FOR UPDATE' : ''}`, [categoryId, gender, prepared.ProductName, prepared.BrandName, prepared.PATTERN || '']);
  return result.rows;
}
async function findProductsByNameBrand(client, prepared, categoryId, gender, lock = false) {
  const result = await client.query(`SELECT id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id
     ...
     WHERE category_id = $1
       ...
       AND LOWER(TRIM(brand_name)) = LOWER(TRIM($4))
     ORDER BY id ASC
     LIMIT 3${lock ? ' FOR UPDATE' : ''}`, [categoryId, gender, prepared.ProductName, prepared.BrandName]);
  return result.rows;
}
function validateMatchedProduct(product, prepared, categoryId, gender) {
  ...
  if (normalizeLogicalText(product.name) !== normalizeLogicalText(prepared.ProductName) || normalizeLogicalText(product.brand_name) !== normalizeLogicalText(prepared.BrandName)) {
...
async function updateProductImportMetadata(client, productId, prepared) {
  const result = await client.query(`UPDATE products
     SET fit_type = COALESCE($1, fit_type),
         ...
         updated_at = NOW()
     WHERE id = $4
     RETURNING id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id, is_active`, [prepared.FITT, prepared.MarkCode, prepared.PatternType, productId]);
  return result.rows[0] || null;
}
async function createProductForImport(client, prepared, categoryId, gender) {
  ...
  const inserted = await client.query(`INSERT INTO products (
     ...
     RETURNING id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id`, [prepared.ProductName, prepared.BrandName, legacyPatternCode, prepared.FITT, prepared.MarkCode, gender, categoryId, requestedDesignCode, prepared.PatternType]);
  ...
  if (!normalizeDesignCode(product.design_code)) {
    ...
    const updated = await client.query(`UPDATE products
       SET design_code = $1,
           pattern_type = COALESCE($2, pattern_type),
           updated_at = NOW()
       WHERE id = $3
       RETURNING id, name, brand_name, design_code, pattern_type, pattern_code, fit_type, mark_code, gender, category_id`, [generatedDesignCode, prepared.PatternType, product.id]);
...
function buildPreparedImportRows(rows, createdStatus, errorStatus) {
  ...
  for (const raw of rows) {
    ...
    const signature = [normalizeLogicalText(prepared.ProductName), normalizeLogicalText(prepared.BrandName), normalizeLogicalText(prepared.PATTERN), normalizeLogicalText(prepared.DesignCode), normalizeLogicalText(prepared.PatternType), normalizeLogicalText(prepared.SIZE), normalizeLogicalText(prepared.COLOUR), String(prepared.MRP ?? ''), String(prepared.RSalePrice ?? ''), String(prepared.CostPrice ?? ''), normalizeLogicalText(prepared.MarkCode), normalizeLogicalText(prepared.FITT)].join('|');
...
async function findImportBarcodeConflicts(preparedRows, categoryId, gender) {
  ...
  for (const existing of result.rows) {
    ...
    const inactiveCategoryMove = categoryMismatch && existing.all_variants_inactive === true && existing.no_active_stock === true && existing.all_product_barcodes_in_import === true;
    const safeCategoryMove = categoryMismatch && (isUncategorizedCategoryPath(existing.category_path) || inactiveCategoryMove) && !genderMismatch && !productMismatch && !brandMismatch && !designMismatch;
    const hardConflict = genderMismatch || productMismatch || brandMismatch || designMismatch || categoryMismatch && !safeCategoryMove;
    if (!hardConflict) {
      continue;
    }
    conflicts.push({
...
function normalizeImportSearchText(value) {
  return cleanText(value).toLowerCase().replace(/co[\s-]*ords?/g, ' coord ').replace(/t[\s-]*shirts?/g, ' tshirt ').replace(/round[\s-]*necks?/g, ' roundneck ').replace(/half[\s-]*sleeves?/g, ' halfsleeve ').replace(/full[\s-]*sleeves?/g, ' fullsleeve ').replace(/3\s*\/\s*4(?:th)?/g, ' threequarter ').replace(/three[\s-]*quarters?/g, ' threequarter ').replace(/night[\s-]*dresses?/g, ' nightdress ').replace(/western[\s-]*wear/g, ' westernwear ').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim();
}
...
function tokenizeImportText(value) {
  ...
  return normalized.split(' ').map(normalizeImportToken).filter(Boolean);
}
...
function getValidPreparedRecords(preparedRows) {
  return preparedRows.filter(item => !item.error_msg).map(item => rowToPreparedRecord(item.raw)).filter(item => item.ProductName && item.BrandName && item.SIZE && item.COLOUR && item.Barcode);
}
...
function scoreImportWorksheetCandidate({
  ...
  const {
    specificTokens,
    specificLeafTokens
  } = getImportCategoryTokens(category);
  const rowText = validRecords.slice(0, 500).map(item => [item.ProductName, item.PATTERN, item.FITT, item.MarkCode].filter(Boolean).join(' ')).join(' ');
  ...
  const genericCategoryTokens = new Set(['top', 'tshirt', 'shirt', 'pant', 'jean', 'cargo', 'jogger', 'short', 'dress', 'nightdress', 'kurti', 'suit', 'polo', 'coord']);
  const discriminatingTokens = specificTokens.filter(token => !genericCategoryTokens.has(token));
  const discriminatingMatches = countTokenMatches(rowTokens, discriminatingTokens) + countTokenMatches(sheetTokens, discriminatingTokens) + countTokenMatches(fileTokens, discriminatingTokens);
  const internalErrorCount = preparedRows.filter(item => item.error_msg).length;
  const audienceMismatch = rowAudience.mismatch || rowAudience.acceptedCount === 0 && sheetAudience.mismatch && fileAudience.mismatch;
  const categoryEvidence = rowCategoryMatches > 0 || rowLeafMatches > 0 || sheetCategoryMatches > 0 || fileCategoryMatches > 0 || discriminatingMatches > 0;
  const score = rowCategoryMatches * 18 + rowLeafMatches * 10 + sheetCategoryMatches * 8 + fileCategoryMatches * 8 + discriminatingMatches * 20 + rowAudience.acceptedCount * 12 + sheetAudience.acceptedCount * 5 + fileAudience.acceptedCount * 5 + Math.min(validRecords.length, 25) - rowAudience.blockedCount * 40 - sheetAudience.blockedCount * 10 - fileAudience.blockedCount * 10 - internalErrorCount * 3 - conflicts.length * 1000;
  return {
    score,
    categoryEvidence,
    audienceMismatch,
    ...
    audienceAccepted: rowAudience.acceptedCount + sheetAudience.acceptedCount + fileAudience.acceptedCount,
    internalErrorCount
  };
}
async function selectImportWorksheet(workbook, category, gender, createdStatus, errorStatus, fileName) {
  const worksheetNames = Array.isArray(workbook.SheetNames) ? workbook.SheetNames.filter(Boolean) : [];
  const candidates = [];
  for (const worksheetName of worksheetNames) {
    ...
    const preparedRows = buildPreparedImportRows(rows, createdStatus, errorStatus);
    const validRecords = getValidPreparedRecords(preparedRows);
    if (!validRecords.length) continue;
    const conflicts = await findImportBarcodeConflicts(preparedRows, category.id, gender);
  ...
  if (!candidates.length) {
    return {
      status: 400,
      error: {
        message: 'No worksheet contains valid rows with ProductName, BrandName, SIZE, COLOUR and Barcode.'
      }
    };
  }
  if (candidates.length === 1) {
    ...
    if (candidate.conflicts.length) {
      return {
        status: 409,
        error: {
          message: 'One or more barcodes already belong to a different product or category. Import was not created.',
    ...
    if (candidate.audienceMismatch) {
      return {
        status: 422,
        error: {
          message: `The uploaded data does not match ${category.category_path}.`,
          detected_worksheet: candidate.name
        }
      };
    }
  ...
  const safeCandidates = candidates.filter(candidate => candidate.conflicts.length === 0 && !candidate.audienceMismatch && categoryCandidateIsSafe(candidate)).sort((a, b) => b.score - a.score || b.discriminatingMatches - a.discriminatingMatches || b.rowCategoryMatches - a.rowCategoryMatches || b.fileCategoryMatches - a.fileCategoryMatches || b.validRecords.length - a.validRecords.length || a.name.localeCompare(b.name));
  if (!safeCandidates.length) {
    const conflictCandidates = candidates.filter(candidate => candidate.conflicts.length > 0 && !candidate.audienceMismatch).sort((a, b) => b.score - a.score || a.conflicts.length - b.conflicts.length);
    if (conflictCandidates.length) {
      const candidate = conflictCandidates[0];
      return {
        status: 409,
        error: {
          message: 'The closest worksheet contains barcodes assigned to another product or category.',
          detected_worksheet: candidate.name,
          conflict_count: candidate.conflicts.length,
          conflicts: candidate.conflicts.slice(0, 50)
        }
    ...
    return {
      status: 422,
      error: {
        message: `The correct worksheet could not be detected for ${category.category_path}.`,
        worksheet_analysis: candidates.map(candidate => ({
          worksheet_name: candidate.name,
          rows: candidate.validRecords.length,
          score: candidate.score,
          category_matches: candidate.rowCategoryMatches + candidate.sheetCategoryMatches + candidate.fileCategoryMatches,
          discriminating_matches: candidate.discriminatingMatches,
          audience_mismatch: candidate.audienceMismatch
        }))
      }
    };
  }
  ...
  if (second) {
    ...
    const sameSignals = top.discriminatingMatches === second.discriminatingMatches && top.rowCategoryMatches === second.rowCategoryMatches && top.sheetCategoryMatches === second.sheetCategoryMatches && top.fileCategoryMatches === second.fileCategoryMatches && top.audienceAccepted === second.audienceAccepted;
    if (scoreGap < 5 && sameSignals) {
      return {
        status: 422,
        error: {
          message: `More than one worksheet appears to match ${category.category_path}.`,
          worksheet_analysis: safeCandidates.slice(0, 5).map(candidate => ({
            worksheet_name: candidate.name,
            rows: candidate.validRecords.length,
            score: candidate.score,
            category_matches: candidate.rowCategoryMatches + candidate.sheetCategoryMatches + candidate.fileCategoryMatches,
...
function categoryCandidateIsSafe(candidate) {
  ...
  return candidate.audienceAccepted > 0 && candidate.validRecords.length > 0;
}
...
function normalizeStorefrontGroupPart(value) {
  const normalized = cleanText(value).toUpperCase().replace(/&/g, ' AND ').replace(/[^A-Z0-9]+/g, '-').replace(/^-+|-+$/g, '');
...
function mergeImages(first, second) {
  ...
  for (const item of [...normalizeImages(first), ...normalizeImages(second)]) {
    const url = String(item?.image_url || item?.imageUrl || item?.secure_url || item?.url || item || '').trim();
...
function mergeVariantPayload(target, source) {
  const existingVariantIds = new Set((target.variant_ids || []).map(value => String(value)));
  const sourceVariantIds = source.variant_ids || [];
  const newVariantIds = sourceVariantIds.filter(value => !existingVariantIds.has(String(value)));
  const shouldAddStock = newVariantIds.length > 0;
  target.variant_ids = uniqueValues([...(target.variant_ids || []), ...sourceVariantIds]);
  target.variantIds = target.variant_ids;
  target.barcodes = uniqueValues([...(target.barcodes || []), ...(source.barcodes || [])]);
  target.ean_codes = target.barcodes;
  target.eanCodes = target.barcodes;
  target.stock_sources = [...(target.stock_sources || []), ...(source.stock_sources || []).filter(item => newVariantIds.some(variantId => String(variantId) === String(item.variant_id)))];
  ...
  if (shouldAddStock) {
    ...
    target.available_qty = toNumber(target.available_qty) + toNumber(source.available_qty);
  ...
  target.front_image_url = target.front_image_url || source.front_image_url || '';
  target.frontImageUrl = target.front_image_url;
  target.back_image_url = target.back_image_url || source.back_image_url || '';
  target.backImageUrl = target.back_image_url;
  target.main_image_url = target.main_image_url || source.main_image_url || '';
...
function groupStockRows(rows) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    const productKey = [normalizeDesignCode(row.design_code) || `PRODUCT-${row.product_id || ''}`, String(row.category_id || '')].join('|');
  ...
  for (const group of productGroups.values()) {
    ...
    const allColours = sortVariantValues(variants.map(variant => variant.colour));
    const allBarcodes = uniqueValues(variants.flatMap(variant => variant.barcodes || []));
    ...
    for (let cardGroupIndex = 0; cardGroupIndex < colourGroupList.length; cardGroupIndex += 1) {
      ...
      const cardVariants = variants.filter(variant => (variant.variant_ids || []).some(variantId => cardVariantIds.has(String(variantId))));
      const cardSizes = sortVariantValues(cardVariants.map(variant => variant.size));
      const cardBarcodes = uniqueValues(cardVariants.flatMap(variant => variant.barcodes || []));
      const cardVariantIdsList = uniqueValues(cardVariants.flatMap(variant => variant.variant_ids || []));
      const stockSources = cardVariants.flatMap(variant => variant.stock_sources || []);
      ...
      const storefrontGroupKey = buildStorefrontGroupKey(group.design_code, group.product_id, selectedColour, group.category_id);
      ...
      groupedProducts.push({
        ...
        stockSources,
        image_url: imageRow.front_image_url || imageRow.main_image_url || imageRow.image_url || '',
        imageUrl: imageRow.front_image_url || imageRow.main_image_url || imageRow.image_url || '',
        front_image_url: imageRow.front_image_url || imageRow.main_image_url || imageRow.image_url || '',
        frontImageUrl: imageRow.front_image_url || imageRow.main_image_url || imageRow.image_url || '',
        back_image_url: imageRow.back_image_url || '',
        backImageUrl: imageRow.back_image_url || '',
        main_image_url: imageRow.main_image_url || imageRow.front_image_url || imageRow.image_url || '',
        mainImageUrl: imageRow.main_image_url || imageRow.front_image_url || imageRow.image_url || '',
...
router.get('/:branchId/import-rows', async (req, res) => {
  ...
  try {
    ...
    const totalResult = await pool.query(`SELECT COUNT(*)::int AS count FROM import_rows WHERE ${whereClause}`, params);
    params.push(limit, offset);
    const rowsResult = await pool.query(`SELECT id, status_enum, error_msg, raw_row_json, category_id FROM import_rows WHERE ${whereClause} ORDER BY id ASC LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
    return res.json({
      job: {
        id: job.id,
...
router.post('/:branchId/import', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (!category) {
      return res.status(400).json({
        message: 'Valid active leaf sub-category is required'
      });
    }
    ...
    const worksheetSelection = await selectImportWorksheet(workbook, category, gender, createdStatus, errorStatus, req.file.originalname || '');
    if (!worksheetSelection.selected) {
      return res.status(worksheetSelection.status || 422).json(worksheetSelection.error || {
        message: 'Unable to detect matching worksheet'
      });
    }
    const worksheetName = worksheetSelection.selected.name;
    const preparedRows = worksheetSelection.selected.preparedRows;
    const barcodeConflicts = worksheetSelection.selected.conflicts;
    if (barcodeConflicts.length) {
      return res.status(409).json({
        message: 'One or more barcodes already belong to a different product or category. Import was not created.',
        worksheet_name: worksheetName,
        conflict_count: barcodeConflicts.length,
        conflicts: barcodeConflicts.slice(0, 50)
      });
    }
    const initialErrorCount = preparedRows.filter(row => row.status_enum === errorStatus).length;
    ...
    const result = await client.query(`INSERT INTO import_jobs (file_name, worksheet_name, file_url, uploaded_by, status_enum, rows_total, rows_success, rows_error, branch_id, gender, category_id, completed_at) VALUES ($1, $2, $3, $4, $5, $6, 0, $7, $8, $9, $10, ${completedAtSql}) RETURNING id, file_name, worksheet_name, file_url, uploaded_by, status_enum, rows_total, rows_success, rows_error, uploaded_at, completed_at, branch_id, gender, category_id`, [fileName, worksheetName, null, null, initialStatus, preparedRows.length, initialErrorCount, branchId, gender, category.id]);
...
router.post('/:branchId/import/process/:jobId', async (req, res) => {
  ...
  const limit = Number.isInteger(requestedLimit) ? Math.max(1, Math.min(PROCESS_MAX_LIMIT, requestedLimit)) : PROCESS_MAX_LIMIT;
  try {
    const branchExists = await ensureBranchExists(branchId);
    if (!branchExists) {
      return res.status(404).json({
    ...
    const jobResult = await pool.query(`SELECT id, file_url, status_enum, rows_total, rows_success, rows_error, gender, category_id FROM import_jobs WHERE id = $1 AND branch_id = $2`, [jobId, branchId]);
    ...
    const errorSamples = [];
    try {
      const batchResult = await client.query(`SELECT id, raw_row_json, category_id FROM import_rows WHERE import_job_id = $1 AND status_enum = $2 ORDER BY id ASC LIMIT $3`, [jobId, createdStatus, limit]);
      const rowsToProcess = batchResult.rows;
      if (!rowsToProcess.length) {
        const countResult = await client.query(`SELECT COUNT(*) FILTER (WHERE status_enum = $2)::int AS ok_count, COUNT(*) FILTER (WHERE status_enum = $3)::int AS error_count FROM import_rows WHERE import_job_id = $1`, [jobId, okStatus, errorStatus]);
        const finalSuccess = countResult.rows[0]?.ok_count || 0;
        const finalError = countResult.rows[0]?.error_count || 0;
        const finalStatus = finalSuccess === 0 && finalError > 0 ? 'FAILED' : finalError > 0 ? 'PARTIAL' : 'COMPLETE';
        await client.query(`UPDATE import_jobs SET rows_success = $1, rows_error = $2, status_enum = $3, completed_at = NOW() WHERE id = $4`, [finalSuccess, finalError, finalStatus, jobId]);
        return res.json({
          done: true,
          processed: 0,
      ...
      const jobBarcodeRows = await client.query(`SELECT raw_row_json FROM import_rows WHERE import_job_id = $1`, [jobId]);
      const jobBarcodes = Array.from(new Set(jobBarcodeRows.rows.map(item => rowToPreparedRecord(item.raw_row_json || {}).Barcode).filter(Boolean)));
      for (const batchRow of rowsToProcess) {
        const raw = batchRow.raw_row_json || {};
        const prepared = rowToPreparedRecord(raw);
        if (!prepared.ProductName || !prepared.BrandName || !prepared.SIZE || !prepared.COLOUR || !prepared.Barcodes.length) {
          const message = 'Missing required fields (ProductName/BrandName/SIZE/COLOUR/Barcode)';
          await client.query(`UPDATE import_rows SET status_enum = $2, error_msg = $3, processed_at = NOW() WHERE id = $1`, [batchRow.id, errorStatus, message]);
          err += 1;
        ...
        try {
          ...
          if (existingBarcodeResult.rowCount) {
            ...
            const safeCategoryMove = categoryMismatch && (isUncategorizedCategoryPath(existing.category_path) || inactiveCategoryMove) && !genderMismatch && !productMismatch && !brandMismatch && !designMismatch;
            if (genderMismatch) {
              throw new Error(`Barcode ${barcode} belongs to gender ${existing.gender}, not ${gender}`);
            }
            if (productMismatch || brandMismatch) {
              throw new Error(`Barcode ${barcode} belongs to ${existing.product_name} / ${existing.brand_name}`);
            }
            if (designMismatch) {
            ...
            if (categoryMismatch && !safeCategoryMove) {
              throw new Error(`Barcode ${barcode} belongs to ${existing.category_path || existing.category_id}, not ${category.category_path}`);
            }
            if (safeCategoryMove) {
              await client.query(`UPDATE products
                   SET category_id = $1,
                       ...
                       delete_batch_id = NULL,
                       updated_at = NOW()
                   WHERE id = $2`, [categoryId, existing.product_id]);
            }
            productId = existing.product_id;
            variantId = existing.variant_id;
            await updateProductImportMetadata(client, productId, prepared);
            await client.query(`UPDATE product_variants SET size = $1, colour = $2, is_active = TRUE, mrp = $3, sale_price = $4, cost_price = $5, b2c_discount_pct = $6, b2b_discount_pct = $7, updated_at = NOW() WHERE id = $8`, [prepared.SIZE, prepared.COLOUR, prepared.MRP, prepared.RSalePrice, prepared.CostPrice, prepared.B2CDiscount, prepared.B2BDiscount, variantId]);
          } else {
            const product = await resolveProductForImport(client, prepared, categoryId, gender);
            productId = product.id;
            const variantResult = await client.query(`INSERT INTO product_variants (product_id, size, colour, is_active, mrp, sale_price, cost_price, b2c_discount_pct, b2b_discount_pct, created_at, updated_at) VALUES ($1, $2, $3, TRUE, $4, $5, $6, $7, $8, NOW(), NOW()) RETURNING id`, [productId, prepared.SIZE, prepared.COLOUR, prepared.MRP, prepared.RSalePrice, prepared.CostPrice, prepared.B2CDiscount, prepared.B2BDiscount]);
            variantId = variantResult.rows[0].id;
            await client.query(`INSERT INTO barcodes (variant_id, ean_code) VALUES ($1, $2)`, [variantId, barcode]);
          }
          await client.query(`INSERT INTO branch_variant_stock (branch_id, variant_id, on_hand, reserved, is_active, created_at, updated_at) VALUES ($1, $2, $3, 0, TRUE, NOW(), NOW()) ON CONFLICT (branch_id, variant_id) DO UPDATE SET on_hand = EXCLUDED.on_hand, reserved = LEAST(branch_variant_stock.reserved, EXCLUDED.on_hand), is_active = TRUE, updated_at = NOW()`, [branchId, variantId, prepared.PurchaseQty]);
          await client.query(`UPDATE import_rows SET status_enum = $2, error_msg = NULL, category_id = $3, processed_at = NOW() WHERE id = $1`, [batchRow.id, okStatus, categoryId]);
          await client.query('COMMIT');
          ok += 1;
        } catch (error) {
          await client.query('ROLLBACK');
    ...
    const statusResult = await pool.query(`SELECT COUNT(*) FILTER (WHERE status_enum = $2)::int AS pending_count, COUNT(*) FILTER (WHERE status_enum = $3)::int AS ok_count, COUNT(*) FILTER (WHERE status_enum = $4)::int AS error_count FROM import_rows WHERE import_job_id = $1`, [jobId, createdStatus, okStatus, errorStatus]);
    ...
    await pool.query(`UPDATE import_jobs SET rows_total = $1, rows_success = $2, rows_error = $3, status_enum = $4, completed_at = CASE WHEN $4 IN ('COMPLETE', 'PARTIAL', 'FAILED') THEN NOW() ELSE completed_at END WHERE id = $5`, [job.rows_total || 0, okCount, errorCount, finalStatus, jobId]);
    const errorCounts = Array.from(errorMap.entries()).map(([message, count]) => ({
      message,
      count
  ...
  try {
    ...
    try {
      ...
      for (const image of images) {
        ...
        const imageType = normalizeImageType(image.image_type || extractImageTypeFromName(image.original_filename || image.public_id || ''));
        ...
        const barcodeResult = await client.query(`SELECT b.ean_code, b.variant_id, v.product_id, v.is_active AS variant_active, bvs.branch_id, bvs.is_active AS stock_active, bvs.on_hand FROM barcodes b JOIN product_variants v ON v.id = b.variant_id LEFT JOIN branch_variant_stock bvs ON bvs.variant_id = v.id AND bvs.branch_id = $2 WHERE REGEXP_REPLACE(UPPER(TRIM(b.ean_code)), '[^A-Z0-9._-]', '', 'g') = $1 ORDER BY CASE WHEN bvs.branch_id = $2 THEN 0 ELSE 1 END, b.id ASC LIMIT 1`, [barcode, branchId]);
        ...
        if (matched.variant_active === false) {
          await client.query(`UPDATE product_variants SET is_active = TRUE, updated_at = NOW() WHERE id = $1`, [matched.variant_id]);
        }
        if (matched.stock_active === false) {
          await client.query(`UPDATE branch_variant_stock SET is_active = TRUE, updated_at = NOW() WHERE branch_id = $1 AND variant_id = $2`, [branchId, matched.variant_id]);
        }
        await client.query(`INSERT INTO product_images (ean_code, image_type, image_url, public_id, uploaded_at) VALUES ($1, $2, $3, $4, NOW()) ON CONFLICT (ean_code, image_type) DO UPDATE SET image_url = EXCLUDED.image_url, public_id = EXCLUDED.public_id, uploaded_at = NOW()`, [matched.ean_code, imageType, imageUrl, publicId || null]);
        if (imageType === 'front' || imageType === 'main') {
          await client.query(`UPDATE product_variants SET image_url = $1, updated_at = NOW() WHERE id = $2`, [imageUrl, matched.variant_id]);
        } else {
          await client.query(`UPDATE product_variants SET image_url = COALESCE(NULLIF(image_url, ''), $1), updated_at = NOW() WHERE id = $2`, [imageUrl, matched.variant_id]);
        }
        updated += 1;
      }
      await client.query('COMMIT');
...
router.get('/:branchId/stock', async (req, res) => {
  ...
  const includeOutOfStock = ['1', 'true', 'yes', 'all'].includes(String(req.query?.include_out_of_stock ?? req.query?.includeOutOfStock ?? '').trim().toLowerCase());
  try {
    const branchExists = await ensureBranchExists(branchId);
    if (!branchExists) {
      return res.status(404).json({
    ...
    let whereClause = `bvs.branch_id = $1 AND bvs.is_active = TRUE AND v.is_active = TRUE AND p.is_active = TRUE AND c.is_active = TRUE`;
    if (!includeOutOfStock) {
      whereClause += ` AND GREATEST(COALESCE(bvs.on_hand, 0) - COALESCE(bvs.reserved, 0), 0) > 0`;
    }
    if (gender) {
      params.push(gender);
...
router.get('/:branchId/discounts', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(`SELECT COALESCE((SELECT v.b2c_discount_pct FROM product_variants v JOIN products p ON p.id = v.product_id JOIN branch_variant_stock bvs ON bvs.variant_id = v.id WHERE bvs.branch_id = $1 AND v.b2c_discount_pct IS NOT NULL AND v.is_active = TRUE AND p.is_active = TRUE AND bvs.is_active = TRUE LIMIT 1), 0) AS b2c_discount_pct, COALESCE((SELECT v.b2b_discount_pct FROM product_variants v JOIN products p ON p.id = v.product_id JOIN branch_variant_stock bvs ON bvs.variant_id = v.id WHERE bvs.branch_id = $1 AND v.b2b_discount_pct IS NOT NULL AND v.is_active = TRUE AND p.is_active = TRUE AND bvs.is_active = TRUE LIMIT 1), 0) AS b2b_discount_pct`, [branchId]);
    if (!result.rows.length) {
      return res.json({
        b2c_discount_pct: 0,
        b2b_discount_pct: 0
...
router.post('/:branchId/discounts', async (req, res) => {
  ...
  if (!Number.isFinite(b2cDiscount) || !Number.isFinite(b2bDiscount) || b2cDiscount < 0 || b2bDiscount < 0 || b2cDiscount > 100 || b2bDiscount > 100) {
    return res.status(400).json({
  ...
  try {
    ...
    await pool.query(`UPDATE product_variants v SET b2c_discount_pct = $2, b2b_discount_pct = $3, updated_at = NOW() FROM branch_variant_stock bvs, products p WHERE bvs.variant_id = v.id AND p.id = v.product_id AND bvs.branch_id = $1 AND v.is_active = TRUE AND p.is_active = TRUE AND bvs.is_active = TRUE`, [branchId, b2cDiscount, b2bDiscount]);
    return res.json({
