...
function shouldSkipBusinessRow(productName, brandName, mrp, salePrice) {
  ...
  const bothZero =
    ...
    (normalizedSalePrice === 0 || normalizedSalePrice === null)
...
function rowToPreparedRecord(raw) {
  ...
  const purchaseQty = Math.max(0, toIntOrZero(row.purchaseqty))
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (existing.signature !== signature) {
      preparedRows.push({
        ...
        error_msg: `Conflicting product details for barcode: ${prepared.Barcode}`
      })
...
function groupStockRows(rows) {
  ...
  for (const group of productGroups.values()) {
    ...
    for (const colourGroup of colourGroups.values()) {
      ...
      const totalOnHand = cardVariants.reduce(
        (sum, variant) => sum + toNumber(variant.on_hand),
        0
      )

      const totalReserved = cardVariants.reduce(
        (sum, variant) => sum + toNumber(variant.reserved),
        0
      )

      const totalAvailable = cardVariants.reduce(
        (sum, variant) => sum + toNumber(variant.available_qty),
        0
      )
  ...
  groupedProducts.sort((a, b) => {
    const categoryCompare = String(a.category_path || '').localeCompare(
...
router.post('/:branchId/import', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const initialErrorCount = preparedRows.filter(
      (row) => row.status_enum === errorStatus
    ).length
...
router.post('/:branchId/import/process/:jobId', async (req, res) => {
  ...
  const requestedLimit = parseInt(
    req.query.limit || String(PROCESS_MAX_LIMIT),
    10
  )
      ...
      if (!rowsToProcess.length) {
        ...
        await client.query(
          `
            ...
            WHERE id = $4
          `,
          [finalSuccess, finalError, finalStatus, jobId]
        )
