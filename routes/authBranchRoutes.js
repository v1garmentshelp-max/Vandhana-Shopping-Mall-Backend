...
router.get('/me', requireAuth, async (req, res) => {
  try {
    const {
      rows
    } = await pool.query('SELECT id, username, role_enum, branch_id, last_login FROM users WHERE id = $1', [req.user.id]);
...
router.post('/change-password', requireAuth, async (req, res) => {
  ...
  try {
    const {
      rows
    } = await pool.query('SELECT hashed_pw FROM users WHERE id = $1', [req.user.id]);
    if (!rows.length) return res.status(404).json({
      message: 'Not found'
    });
    let ok = false;
...
router.get('/branch-admins', requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    const {
      rows
    } = await pool.query("SELECT id, username, role_enum, branch_id, last_login, is_active, name FROM users WHERE role_enum::text LIKE 'BRANCH%' ORDER BY id DESC");
...
router.post('/branch-admins', requireAuth, requireSuperAdmin, async (req, res) => {
  ...
  if (!email || !password || !branch_id) {
    return res.status(400).json({
      message: 'email, password and branch_id required'
    });
  }
  try {
    const existing = await pool.query('SELECT id FROM users WHERE username = $1 LIMIT 1', [email]);
    if (existing.rows.length) {
      return res.status(409).json({
        message: 'Admin with this email already exists'
      });
    }
    const branchCheck = await pool.query('SELECT id FROM branches WHERE id = $1 LIMIT 1', [branch_id]);
    ...
    const {
      rows
    } = await pool.query(`INSERT INTO users (username, hashed_pw, role_enum, branch_id, is_active, name)
       VALUES ($1, $2, $3, $4, true, $5)
       RETURNING id, username, role_enum, branch_id, last_login, is_active, name`, [email, hashed, roleEnum, branch_id, name || null]);
...
router.put('/branch-admins/:id', requireAuth, requireSuperAdmin, async (req, res) => {
  ...
  if (!email || !branch_id) {
    return res.status(400).json({
      message: 'email and branch_id required'
    });
  }
  try {
    const {
      rows: existingRows
    } = await pool.query("SELECT * FROM users WHERE id = $1 AND role_enum::text LIKE 'BRANCH%'", [id]);
    if (!existingRows.length) return res.status(404).json({
      message: 'Branch admin not found'
    });
    const usernameTaken = await pool.query('SELECT id FROM users WHERE username = $1 AND id <> $2 LIMIT 1', [email, id]);
    ...
    const {
      rows
       ...
       RETURNING id, username, role_enum, branch_id, last_login, is_active, name`, params);
    if (!rows.length) return res.status(404).json({
      message: 'Branch admin not found'
    });
  ...
  try {
    const {
      rows
       ...
       RETURNING id, username, role_enum, branch_id, last_login, is_active, name`, [id]);
    if (!rows.length) return res.status(404).json({
      message: 'Branch admin not found'
    });
    return res.json(mapUserToBranchAdmin(rows[0]));
