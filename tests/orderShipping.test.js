...
const sr = {
  ...
  api: async (method, endpoint, payload) => {
    if (endpoint.startsWith('/orders/show/')) return {
      data: {
        data: {
          id: 900,
          channel_order_id: remoteSale,
          shipments: {
            id: 901,
            awb: remoteAwb,
            weight: 0.5
    ...
    if (endpoint === '/courier/assign/awb') {
      ...
      if (mode === 'wallet') return {
        data: {
          ...
          response: {
            data: {
              awb_assign_error: 'Recharge wallet'
            }
          }
        }
      };
...
async function sale() {
  ...
  await db.query(`INSERT INTO sales(id,source,status,payment_method,payment_status,branch_id,total,customer_name,customer_mobile,shipping_address)
    VALUES($1,'WEB','PLACED','COD','COD',3,545,'Test','9999999999',$2)`, [id, JSON.stringify({
    line1: 'Test address',
    city: 'Test City',
    state: 'Test State',
    pincode: '560037'
...
test('linking validates channel order ID and recovers without creating', async () => {
  ...
  remoteSale = id;
  const result = await flow.connect(id, {
    remote_order_id: 900
  });
...
test('concurrent shipping actions acquire only one order lock', async () => {
  const id = await sale();
  const n = creates;
  const results = await Promise.allSettled([flow.connect(id, {
    fresh: true
  }), flow.connect(id, {
    fresh: true
  })]);
...
test('return windows use a carrier delivery timestamp, never an estimate or the tracking refresh time', async () => {
  ...
  let rows = (await db.query('SELECT * FROM shipments WHERE sale_id=$1', [id])).rows;
  const estimated = {
    current_status: 'DELIVERED',
    expected_delivery_date: '2026-09-22T12:00:00Z'
  };
  assert.equal(extractShipmentInfo(estimated).delivered_at, null);
  let result = await syncShipmentRows(pool, rows, estimated);
  assert.equal(result[0].status, 'DELIVERED');
  assert.equal(result[0].delivered_at, null);
  result = await syncShipmentRows(pool, result, {
    current_status: 'DELIVERED',
    delivered_date: '2026-09-21T12:00:00Z'
  });
