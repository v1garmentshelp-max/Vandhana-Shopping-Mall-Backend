...
const { getSettings, creditSignupBonus } = require('../services/rewardPointsService')
...
async function main() {
  ...
  console.log(JSON.stringify({ user: user.rows[0], settings, existing_signup_bonus: existing.rows }, null, 2))
  ...
  console.log(result ? JSON.stringify({ signup_bonus: result }, null, 2) : 'No credit: rewards are disabled, bonus is zero, or the account is not B2C.')
}
