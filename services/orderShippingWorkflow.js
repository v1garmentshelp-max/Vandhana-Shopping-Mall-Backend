...
function createWorkflow(pool, makeClient = () => new Shiprocket({
  pool
})) {
  ...
  async function locked(id, action) {
    ...
    try {
      ...
      if (acquired) {
        try {
          await db.query('SELECT pg_advisory_unlock(hashtextextended($1::text,0))', [`order-shipping:${id}`]);
        } catch (e) {
          db.release(true);
          acquired = false;
          throw e;
  ...
  async function persist(db, sale, orderId, shipmentId, awb) {
    ...
    if (!existing) {
      await db.query(`INSERT INTO shipments(id,sale_id,branch_id,shiprocket_order_id,shiprocket_shipment_id,awb,status,raw_status)
        VALUES($1,$2,$3,$4,$5,$6,'CONFIRMED','NEW')`, [crypto.randomUUID(), sale.id, sale.branch_id, String(orderId), String(shipmentId), awbText(awb)]);
    } else if (awbText(awb)) {
  ...
  async function remoteDetail(sr, sale, id) {
    ...
    const {
      data
    } = await sr.api('get', `/orders/show/${id}`);
  ...
  async function connect(id, {
    ...
    return locked(id, async (db, s) => {
      ...
      const items = (await db.query('SELECT si.*,COALESCE(si.custom_title,p.name) AS name FROM sale_items si LEFT JOIN products p ON p.id=si.product_id WHERE sale_id=$1', [id])).rows;
      ...
      let data;
      try {
        data = await sr.createOrderShipment({
          channel_order_id: id,
          pickup_location: s.warehouse.name,
          order: {
            items,
          ...
          customer: {
            name: s.sale.customer_name,
            email: s.sale.customer_email,
            phone,
            address
          }
        });
  ...
  async function couriers(id) {
    ...
    const data = await sr.checkServiceability({
      pickup_postcode: s.warehouse.pincode,
      delivery_postcode: addressOf(s.sale).pincode,
      cod: String(s.sale.payment_method).toUpperCase() === 'COD',
      weight
    });
    return {
      ...data,
  ...
  async function assignAwb(id, courierId) {
    return locked(id, async (db, s) => {
      ...
      const available = await sr.checkServiceability({
        pickup_postcode: s.warehouse?.pincode,
        delivery_postcode: addressOf(s.sale).pincode,
        cod: String(s.sale.payment_method).toUpperCase() === 'COD',
        weight: Number(remote.shipment.weight) || 0.5
      });
      ...
      const {
        data
      } = await sr.api('post', '/courier/assign/awb', {
        shipment_id: Number(remote.shipmentId),
  ...
  async function pickup(id) {
    return locked(id, async (db, s) => {
      ...
      const result = await sr.requestPickup({
        shipment_id: Number(s.shipment.shiprocket_shipment_id)
      });
  ...
  async function tracking(id) {
    ...
    const {
      data
    } = await makeClient().api('get', `/courier/track/shipment/${s.shipment.shiprocket_shipment_id}`);
    ...
    await syncShipmentByIdentifiers(pool, {
      sale_id: id,
      shiprocket_shipment_id: s.shipment.shiprocket_shipment_id
    }, {
    ...
    return {
      ...
      awb: current.awb_code || s.shipment.awb,
  ...
  async function document(id, type) {
    ...
    const routes = {
      ...
      invoice: ['/orders/print/invoice', {
        ids: [Number(s.shipment.shiprocket_order_id)]
      }, 'invoice_url'],
