...
const failure = (message, status = 409, available = 0) => Object.assign(new Error(message), { status, available })

async function writeStockCart({ userId, variantId, cartItemId, quantity, branchId, size, color, add }) {
  ...
  try {
    ...
    if (!add) {
      const q = cartItemId
        ? await client.query('SELECT * FROM vandana_cart WHERE id = $1 AND user_id = $2 FOR UPDATE', [cartItemId, userId])
        : await client.query('SELECT * FROM vandana_cart WHERE user_id = $1 AND product_id = $2 AND selected_size = $3 AND selected_color = $4 AND is_custom = FALSE FOR UPDATE', [userId, variantId, size, color])
      target = q.rows[0]
