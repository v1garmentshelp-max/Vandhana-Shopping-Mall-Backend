...
class Shiprocket {
  ...
  async login() {
    ...
    const {
      data
    } = await axios.post(`${ROOT.replace(/\/+$/, '')}/v1/external/auth/login`, {
      email,
      password
    }, {
      timeout: 12000
  ...
  async upsertWarehouseFromBranch(branch) {
    ...
    const {
      data
    } = await this.api('post', '/settings/company/addpickup', payload);
    return data;
  }
  async createOrderShipment({
    ...
    const payload = {
      ...
      order_items: (order.items || []).map(it => ({
        ...
        sku: String(it.variant_id || it.id),
    ...
    const {
      data
    } = await this.api('post', '/orders/create/adhoc', payload);
    return data;
  }
  async assignAWBAndLabel({
    shipment_id
  }) {
    const ids = Array.isArray(shipment_id) ? shipment_id : [shipment_id];
    const {
      data: awb
    } = await this.api('post', '/courier/assign/awb', {
      shipment_id: ids[0]
    });
    const {
      data: label
    } = await this.api('post', '/courier/generate/label', {
      shipment_id: ids
    });
    return {
      awb,
  ...
  async requestPickup({
    ...
    const {
      data
    } = await this.api('post', '/courier/generate/pickup', payload);
    return data;
  }
  async generateManifest({
    ...
    const {
      data
    } = await this.api('post', '/manifests/generate', {
      shipment_id: ids
    });
    return data;
  }
  async checkServiceability({
    ...
    const {
      data
    } = await this.api('get', '/courier/serviceability', params);
    return data;
  }
  async cancelOrders({
    order_ids
    ...
    const {
      data
    } = await this.api('post', '/orders/cancel', {
      ids
    });
    return data;
  }
