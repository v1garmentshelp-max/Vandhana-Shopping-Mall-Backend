...
const getDeletedBy = req => {
  return cleanText(req?.user?.email || req?.user?.username || req?.user?.name || req?.user?.id || req?.body?.deleted_by || req?.body?.deletedBy || req?.headers?.['x-admin-user'] || '') || null;
};
...
const lockActiveProducts = async (client, productIds) => {
  ...
  const result = await client.query(`SELECT id, name, brand_name, design_code
     FROM products
...
const getReservedByProduct = async (client, productIds) => {
  ...
  const result = await client.query(`SELECT
     ...
     GROUP BY v.product_id`, [productIds]);
...
const insertBatch = async ({
  ...
  const result = await client.query(`INSERT INTO product_delete_batches (
       delete_scope,
       requested_count,
       deleted_count,
       blocked_count,
     ...
     RETURNING id`, [scope, requestedCount, deletedBy]);
...
const completeBatch = async ({
  ...
  await client.query(`UPDATE product_delete_batches
     SET deleted_count = $2,
         blocked_count = $3,
         completed_at = NOW()
     WHERE id = $1`, [batchId, deletedCount, blockedCount]);
};
const deleteProducts = async ({
  ...
  try {
    ...
    const requestedIds = normalizedScope === 'all' ? await getAllCandidateIds(client, filters) : uniqueIds(productIds);
    ...
    if (deletableIds.length) {
      await client.query(`UPDATE branch_variant_stock bvs
         SET is_active = FALSE,
             updated_at = NOW()
         FROM product_variants v
         WHERE v.id = bvs.variant_id
           AND v.product_id = ANY($1::bigint[])`, [deletableIds]);
      await client.query(`UPDATE product_variants
         SET is_active = FALSE,
             updated_at = NOW()
         WHERE product_id = ANY($1::bigint[])`, [deletableIds]);
      const deleted = await client.query(`UPDATE products
         SET is_active = FALSE,
             deleted_at = NOW(),
             delete_batch_id = $2,
             updated_at = NOW()
         WHERE id = ANY($1::bigint[])
           AND is_active = TRUE
         RETURNING id`, [deletableIds, batchId]);
