...
async function getSetting(key, db = pool) {
  const q = await db.query(`SELECT setting_value
...
async function getSettings(db = pool) {
  const q = await db.query(`SELECT setting_key, setting_value
     FROM reward_settings
...
async function expireLots(db = pool, userId = null) {
  ...
  const q = await db.query(`UPDATE reward_point_lots
     SET status = 'EXPIRED',
         updated_at = NOW()
     WHERE status = 'ACTIVE'
       ...
       AND expires_at <= NOW()
       ${userSql}
     RETURNING id, user_id, points_remaining`, params);
  for (const row of q.rows) {
    await db.query(`INSERT INTO reward_point_transactions
       (user_id, lot_id, transaction_type, points, note, metadata, created_at)
       VALUES ($1, $2, 'EXPIRED', $3, $4, $5::jsonb, NOW())`, [row.user_id, row.id, -Math.abs(Number(row.points_remaining || 0)), 'Reward points expired', JSON.stringify({
      expired_points: Number(row.points_remaining || 0)
    })]);
  }
...
async function creditSignupBonus(userId, db = pool) {
  ...
  if (!uid) throw Object.assign(new Error('Invalid user id'), {
    status: 400
  });
  const settings = await getSettings(db);
  if (!settings.enabled || settings.signup_bonus_points <= 0) return null;
  const userQ = await db.query(`SELECT id, type
     FROM vandana_users
     WHERE id = $1
     FOR UPDATE`, [uid]);
  ...
  const existing = await db.query(`SELECT id, user_id, points_granted, points_remaining, granted_at, expires_at, status
     FROM reward_point_lots
     WHERE user_id = $1
       AND source_type = 'SIGNUP_BONUS'
     LIMIT 1`, [uid]);
  if (existing.rowCount) return existing.rows[0];
  const lotQ = await db.query(`INSERT INTO reward_point_lots
     ...
     RETURNING id, user_id, points_granted, points_remaining, granted_at, expires_at, status`, [uid, settings.signup_bonus_points, settings.validity_days]);
  const lot = lotQ.rows[0];
  await db.query(`INSERT INTO reward_point_transactions
     (user_id, lot_id, transaction_type, points, note, metadata, created_at)
     VALUES ($1, $2, 'SIGNUP_BONUS', $3, $4, $5::jsonb, NOW())`, [uid, lot.id, settings.signup_bonus_points, 'Signup reward points', JSON.stringify({
...
async function getWalletSummary(userId, db = pool) {
  ...
  const userQ = await db.query(`SELECT id, name, email, type
     FROM vandana_users
     WHERE id = $1
     LIMIT 1`, [uid]);
  if (!userQ.rowCount) throw Object.assign(new Error('User not found'), {
  ...
  const lotsQ = await db.query(`SELECT
     ...
     FROM reward_point_lots
     WHERE user_id = $1
     ORDER BY expires_at ASC, id ASC`, [uid]);
  const activeLots = lotsQ.rows.filter(row => row.status === 'ACTIVE' && Number(row.points_remaining || 0) > 0);
  const balance = activeLots.reduce((sum, row) => sum + Number(row.points_remaining || 0), 0);
  const nearest = activeLots[0] || null;
  const expiringSoon = activeLots.filter(row => Number(row.days_remaining) <= settings.warning_days);
  const expiringSoonPoints = expiringSoon.reduce((sum, row) => sum + Number(row.points_remaining || 0), 0);
  return {
    ...
    hurry_up: Boolean(nearest && Number(nearest.days_remaining) <= settings.warning_days),
...
async function getHistory(userId, limit = 50, db = pool) {
  ...
  await expireLots(db, uid);
  const safeLimit = Math.min(Math.max(asPositiveInt(limit) || 50, 1), 200);
...
async function getRedeemableBalance(userId, db = pool, lock = false) {
  ...
  await expireLots(db, uid);
  const q = await db.query(`SELECT id, user_id, points_granted, points_remaining, granted_at, expires_at, status
     FROM reward_point_lots
     WHERE user_id = $1
       AND status = 'ACTIVE'
       AND points_remaining > 0
     ...
     ${lock ? 'FOR UPDATE' : ''}`, [uid]);
  const balance = q.rows.reduce((sum, row) => sum + Number(row.points_remaining || 0), 0);
  return {
    balance,
    lots: q.rows
  };
...
async function previewRedemption({
  ...
  if (points == null) {
    throw Object.assign(new Error('reward_points must be a non-negative integer'), {
      status: 400
    });
  }
  const subtotal = asMoney(orderSubtotal);
  if (subtotal < 0) {
    throw Object.assign(new Error('Invalid order subtotal'), {
      status: 400
    });
  }
  const settings = await getSettings(db);
  const wallet = await getRedeemableBalance(userId, db, false);
  const maxByOrder = Math.max(Math.floor(subtotal), 0);
  const maxRedeemable = Math.min(wallet.balance, maxByOrder);
  return {
    enabled: settings.enabled,
    balance: wallet.balance,
    requested_points: points,
    max_redeemable: settings.enabled ? maxRedeemable : 0,
    can_redeem: settings.enabled && points <= maxRedeemable,
    reward_discount: settings.enabled && points <= maxRedeemable ? points : 0,
    payable_after_rewards: settings.enabled && points <= maxRedeemable ? asMoney(Math.max(subtotal - points, 0)) : subtotal,
    point_value_rupees: 1
  };
}
async function redeemPoints(db, {
  userId,
  requestedPoints,
  saleId,
  orderSubtotal
  ...
  if (!points) {
    throw Object.assign(new Error('reward_points must be a positive integer'), {
      status: 400
    });
  }
  if (!saleId) {
    throw Object.assign(new Error('sale_id required'), {
      status: 400
    });
  }
  const settings = await getSettings(db);
  if (!settings.enabled) {
    throw Object.assign(new Error('Reward points are disabled'), {
      status: 409,
      code: 'REWARDS_DISABLED'
    });
  }
  const wallet = await getRedeemableBalance(uid, db, true);
  const maxByOrder = Math.max(Math.floor(subtotal), 0);
  const maxRedeemable = Math.min(wallet.balance, maxByOrder);
  if (points > wallet.balance) {
    throw Object.assign(new Error('Insufficient reward points'), {
      status: 409,
  ...
  if (points > maxByOrder) {
    throw Object.assign(new Error('Reward points cannot exceed the order payable amount'), {
      status: 409,
  ...
  for (const lot of wallet.lots) {
    ...
    const lotAvailable = Number(lot.points_remaining || 0);
    const deduct = Math.min(lotAvailable, remaining);
    if (deduct <= 0) continue;
    const updated = await db.query(`UPDATE reward_point_lots
       SET points_remaining = points_remaining - $2,
           ...
           updated_at = NOW()
       WHERE id = $1
       RETURNING id, points_remaining, status`, [lot.id, deduct]);
    await db.query(`INSERT INTO reward_point_transactions
       (user_id, lot_id, sale_id, transaction_type, points, note, metadata, created_at)
       VALUES
       ($1, $2, $3::uuid, 'REDEEMED', $4, $5, $6::jsonb, NOW())`, [uid, lot.id, saleId, -deduct, 'Reward points redeemed', JSON.stringify({
    ...
    deductions.push({
      ...
      points: deduct,
      expires_at: lot.expires_at,
      points_remaining: Number(updated.rows[0].points_remaining || 0)
    });
    remaining -= deduct;
  }
  if (remaining > 0) {
...
async function releaseRewardsForSale(db, saleId) {
  ...
  await db.query('SELECT pg_advisory_xact_lock(hashtext($1))', [String(saleId)]);
  const redeemedQ = await db.query(`SELECT
       t.user_id,
       t.lot_id,
  ...
  for (const row of redeemedQ.rows) {
    const refundedQ = await db.query(`SELECT
       ...
       WHERE sale_id = $1::uuid
         ...
         AND points > 0`, [saleId, row.user_id, row.lot_id]);
    const redeemedPoints = Number(row.redeemed_points || 0);
    const alreadyRefunded = Number(refundedQ.rows[0]?.refunded_points || 0);
    const outstanding = Math.max(redeemedPoints - alreadyRefunded, 0);
    if (outstanding <= 0) continue;
    const lotQ = await db.query(`SELECT
         id,
         user_id,
         points_granted,
         points_remaining,
    ...
    await db.query(`UPDATE reward_point_lots
       SET points_remaining = points_remaining + $2,
           ...
           END,
           updated_at = NOW()
       WHERE id = $1`, [lot.id, restorePoints, activeAfterRestore]);
    await db.query(`INSERT INTO reward_point_transactions
       (user_id, lot_id, sale_id, transaction_type, points, note, metadata, created_at)
       VALUES
       ($1, $2, $3::uuid, 'REFUNDED', $4, $5, $6::jsonb, NOW())`, [row.user_id, lot.id, saleId, restorePoints, 'Reward points restored', JSON.stringify({
    ...
    if (!activeAfterRestore) {
      await db.query(`INSERT INTO reward_point_transactions
         (user_id, lot_id, sale_id, transaction_type, points, note, metadata, created_at)
         VALUES
         ($1, $2, $3::uuid, 'EXPIRED', $4, $5, $6::jsonb, NOW())`, [row.user_id, lot.id, saleId, -restorePoints, 'Restored reward points were already past their original expiry', JSON.stringify({
        original_expiry: lot.expires_at
      })]);
    }
    restored.push({
  ...
  return {
    restored_points: restored.reduce((sum, item) => sum + item.points, 0),
    restored
  };
}
async function releaseRewardsForSaleWithTransaction(saleId) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const result = await releaseRewardsForSale(client, saleId);
    await client.query('COMMIT');
    return result;
  } catch (error) {
    try {
