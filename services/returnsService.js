  ...
  async createReversePickup({
    ...
    const payload = {
      ...
      order_items: items.map(it => ({
        name: `Return ${it.variant_id || it.sale_item_id}`,
        sku: String(it.variant_id || it.sale_item_id),
        units: it.qty,
        selling_price: 0
      })),
    ...
    const {
      data
    } = await this.sr.api('post', '/orders/create/adhoc', payload);
    const shipmentId = Array.isArray(data?.shipment_id) ? data.shipment_id[0] : data?.shipment_id || null;
    let awb = null,
      labelUrl = null;
    if (shipmentId) {
      const res = await this.sr.assignAWBAndLabel({
        shipment_id: shipmentId
      });
      awb = res.awb?.response?.data?.awb_code || null;
      labelUrl = res.label?.label_url || null;
    }
    const ins = await this.pool.query(`INSERT INTO reverse_shipments (request_id, shiprocket_order_id, shiprocket_shipment_id, awb, label_url, tracking_url, status)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       RETURNING *`, [request.id, data?.order_id || null, shipmentId, awb, labelUrl, data?.tracking_url || null, 'CREATED']);
    return ins.rows[0];
  }
}
