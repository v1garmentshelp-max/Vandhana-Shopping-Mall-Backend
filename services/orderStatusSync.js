...
const findFirstByKeys = (input, keys, depth = 0, exact = false) => {
  if (!input || depth > 8) return null;
  if (Array.isArray(input)) {
    for (const item of input) {
      const found = findFirstByKeys(item, keys, depth + 1, exact);
  ...
  for (const [key, value] of Object.entries(input)) {
    const k = String(key || '').toLowerCase();
    if (keys.some(x => k === x || !exact && k.includes(x))) {
      if (value != null && value !== '') return value;
    }
  }
  for (const value of Object.values(input)) {
    if (value && typeof value === 'object') {
      const found = findFirstByKeys(value, keys, depth + 1, exact);
      if (found != null && found !== '') return found;
    }
  }
  return null;
  ...
  for (const row of rows || []) {
    ...
    const q = await pool.query(`UPDATE shipments
       SET status = COALESCE($2, status),
           ...
           updated_at = now()
       WHERE id = $1
       RETURNING *`, [row.id, nextStatus || null, info.raw_status || null, info.awb || '', info.tracking_url || '', info.label_url || '', info.current_location || '', JSON.stringify(info.raw || raw || {}), deliveredAt && !Number.isNaN(deliveredAt.getTime()) ? deliveredAt.toISOString() : null]);
